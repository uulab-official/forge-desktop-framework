...
- Keep `.github/workflows/release.yml` aligned too: packaged release jobs should emit human-readable inventories, publish a final matrix summary, and fail early when signing secrets, platform-specific installers, or `latest*.yml` manifests are missing.
