...
- Keep the final `one-point-zero-major-release-constitution.md/json`, `one-point-zero-major-release-covenant.md/json`, `one-point-zero-major-release-compact.md/json`, and `one-point-zero-major-release-capsule.md/json` summaries aligned with the canon artifact and prepared `v1.0.0` checklist so the first `1.0.0` immutable constitution, covenant, compact, and capsule stay reproducible.
