...
That starter now also includes `pnpm security:check`, `pnpm ops:check`, `pnpm ops:snapshot`, `pnpm ops:evidence`, `pnpm production:check` for the default GitHub path, and `pnpm production:check:all -- --require-release-output` for a full post-package audit. Its generated validate and tagged release workflows also upload both `ops/snapshots/` and `ops/evidence/` as build artifacts.
...
- `.env.example`, `docs/release-playbook.md`, `docs/production-readiness.md`, `pnpm release:check`, `pnpm security:check`, `pnpm ops:check`, `pnpm ops:snapshot`, `pnpm ops:evidence`, `pnpm publish:check:*`, `pnpm package:verify*`, `pnpm package:audit*`, and `pnpm production:check*`
