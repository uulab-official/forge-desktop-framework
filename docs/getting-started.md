...
That starter now also includes `pnpm security:check`, `pnpm production:check` for the default GitHub path, and `pnpm production:check:all -- --require-release-output` for a full post-package audit.
...
- `.env.example`, `docs/release-playbook.md`, `docs/production-readiness.md`, `pnpm release:check`, `pnpm security:check`, `pnpm publish:check:*`, `pnpm package:verify*`, `pnpm package:audit*`, and `pnpm production:check*`
