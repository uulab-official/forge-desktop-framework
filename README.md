...
Generated apps from that preset now also ship `pnpm security:check`, `pnpm ops:check`, `pnpm ops:snapshot`, `pnpm ops:evidence`, `pnpm ops:report`, `pnpm ops:bundle`, `pnpm ops:index`, `pnpm ops:doctor`, `pnpm ops:handoff`, `pnpm ops:attest`, `pnpm ops:ready`, `pnpm ops:retention`, `pnpm production:check` for the default GitHub release path, and `pnpm production:check:all -- --require-release-output` for a full post-package audit. The generated validate and tagged release workflows now also upload `ops/snapshots/`, `ops/evidence/`, `ops/reports/`, `ops/bundles/`, `ops/index/`, `ops/doctors/`, `ops/handoffs/`, `ops/attestations/`, and `ops/ready/` as GitHub Actions artifacts, while `ops:retention` keeps those directories bounded during repeated production checks.
...
- GitHub Actions artifact upload for generated `ops/snapshots/`, `ops/evidence/`, `ops/reports/`, `ops/bundles/`, `ops/index/`, `ops/doctors/`, `ops/handoffs/`, `ops/attestations/`, and `ops/ready/` operator evidence from validate and tagged release runs
