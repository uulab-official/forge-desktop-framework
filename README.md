...
Tagged release metadata now also emits `one-point-zero-major-release-constitution.md/json`, which turns that canon artifact plus prepared `v1.0.0` checklist into the final immutable constitution surface before the first major ship.
...
The same `1.0` stack now also emits `one-point-zero-major-release-constitution.md/json` so the final immutable major-release constitution surface survives as a tagged release artifact too.
