...
Generated apps from that preset now also ship `pnpm security:check`, `pnpm ops:check`, `pnpm ops:snapshot`, `pnpm ops:evidence`, `pnpm production:check` for the default GitHub release path, and `pnpm production:check:all -- --require-release-output` for a full post-package audit. The generated validate and tagged release workflows now also upload both `ops/snapshots/` and `ops/evidence/` as GitHub Actions artifacts.
...
- `docs/production-readiness.md` plus `pnpm security:check`, `pnpm ops:check`, `pnpm ops:snapshot`, `pnpm ops:evidence`, and `pnpm production:check*` commands for repeatable pre-ship validation
- GitHub Actions artifact upload for generated `ops/snapshots/` and `ops/evidence/` operator evidence from validate and tagged release runs
