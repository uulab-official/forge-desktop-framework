...
Tagged release metadata now also emits `one-point-zero-major-release-constitution.md/json` and `one-point-zero-major-release-covenant.md/json`, which turn that canon artifact and prepared `v1.0.0` checklist into the final immutable constitution and covenant surfaces before the first major ship.
...
The same `1.0` stack now also emits `one-point-zero-major-release-constitution.md/json` and `one-point-zero-major-release-covenant.md/json` so the final immutable major-release constitution and covenant surfaces survive as tagged release artifacts too.
