...
- `docs/production-readiness.md` plus `pnpm security:check`, `pnpm ops:check`, `pnpm ops:snapshot`, `pnpm ops:evidence`, `pnpm ops:report`, `pnpm ops:bundle`, `pnpm ops:index`, `pnpm ops:retention`, and `pnpm production:check*` commands for repeatable pre-ship validation
- GitHub Actions artifact upload for generated `ops/snapshots/`, `ops/evidence/`, `ops/reports/`, `ops/bundles/`, and `ops/index/` operator evidence from validate and tagged release runs
