...
- `docs/production-readiness.md` plus `pnpm security:check`, `pnpm ops:check`, `pnpm ops:snapshot`, `pnpm ops:evidence`, `pnpm ops:index`, `pnpm ops:report`, `pnpm ops:retention`, and `pnpm production:check*` commands for repeatable pre-ship validation
- GitHub Actions artifact upload for generated `ops/snapshots/`, `ops/evidence/`, `ops/index/`, and `ops/reports/` operator evidence from validate and tagged release runs
