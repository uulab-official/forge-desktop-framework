...
- The final `one-point-zero-major-release-constitution`, `one-point-zero-major-release-covenant`, `one-point-zero-major-release-compact`, and `one-point-zero-major-release-capsule` artifacts should stay in step with the canon artifact and prepared `v1.0.0` checklist so the first `1.0.0` immutable constitution, covenant, compact, and capsule remain reproducible.
