...
- The final `one-point-zero-major-release-constitution`, `one-point-zero-major-release-covenant`, and `one-point-zero-major-release-compact` artifacts should stay in step with the canon artifact and prepared `v1.0.0` checklist so the first `1.0.0` immutable constitution, covenant, and compact remain reproducible.
