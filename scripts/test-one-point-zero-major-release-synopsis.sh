...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/forge-one-point-zero-major-synopsis.XXXXXX")"
...
cat > "$WORK_DIR/one-point-zero-release-candidate.json" <<'EOF'
{
  ...
  "currentVersion": "0.1.113",
  "currentTag": "v0.1.113",
  "currentCommit": "deadbeef113",
  "nextVersion": "1.0.0",
  ...
  "checklist": {
    ...
    "userFacingChange": "maintainers now get one-point-zero-major-release-synopsis.md/json as the final synopsis surface before the first 1.0.0 ship.",
    "followUpRisks": "the synopsis artifact must stay aligned with the digest artifact and prepared v1.0.0 checklist",
    "nextTarget": "use the synopsis artifact as the last immutable synopsis before pnpm release:ship major"
  },
  "officialPresets": [
    { "preset": "launch-ready", "passed": true },
    { "preset": "support-ready", "passed": true },
...
[[ -f "$WORK_DIR/output/one-point-zero-major-release-synopsis.md" ]]
[[ -f "$WORK_DIR/output/one-point-zero-major-release-synopsis.json" ]]

node - "$WORK_DIR/output/one-point-zero-major-release-synopsis.json" <<'NODE'
const fs = require('node:fs');
const payload = JSON.parse(fs.readFileSync(process.argv[2], 'utf8'));

if (payload.synopsis !== 'ready-for-major-release-synopsis') {
  throw new Error(`Expected synopsis ready-for-major-release-synopsis, found ${payload.synopsis}`);
}
...
for (const command of [
  ...
  if (!payload.validationCommands.includes(command) && !payload.shipCommands.includes(command)) {
    throw new Error(`Missing expected command in synopsis artifact: ${command}`);
  }
}
NODE

echo "One Point Zero major release synopsis smoke test passed."
