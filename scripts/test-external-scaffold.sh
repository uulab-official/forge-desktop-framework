...
verify_external_app() {
  ...
  if [[ "$preset_id" == "production-ready" ]]; then
    ...
    if ! find "$target_dir/ops/certifications" -name 'ops-certify.json' -print -quit | grep -q .; then
      echo "External ${preset_id} smoke app ops certify JSON was not produced."
      exit 1
    fi
    if ! find "$target_dir/ops/certifications" -name 'ops-certify.md' -print -quit | grep -q .; then
      echo "External ${preset_id} smoke app ops certify Markdown was not produced."
      exit 1
    fi
    if ! find "$target_dir/ops/certifications" -name 'ops-certify.tgz' -print -quit | grep -q .; then
      echo "External ${preset_id} smoke app ops certify archive was not produced."
      exit 1
    fi
    if ! find "$target_dir/ops/certifications" -path '*/packet/compliance/ops-compliance.json' -print -quit | grep -q .; then
      echo "External ${preset_id} smoke app ops certify packet did not capture the latest compliance JSON."
      exit 1
    fi
    if ! find "$target_dir/ops/assurances" -name 'ops-assure.json' -print -quit | grep -q .; then
      echo "External ${preset_id} smoke app ops assure JSON was not produced."
      exit 1
    fi
    if ! find "$target_dir/ops/assurances" -name 'ops-assure.md' -print -quit | grep -q .; then
      echo "External ${preset_id} smoke app ops assure Markdown was not produced."
      exit 1
    fi
    if ! find "$target_dir/ops/assurances" -name 'ops-assure.tgz' -print -quit | grep -q .; then
      echo "External ${preset_id} smoke app ops assure archive was not produced."
      exit 1
    fi
    if ! find "$target_dir/ops/assurances" -path '*/packet/certification/ops-certify.json' -print -quit | grep -q .; then
      echo "External ${preset_id} smoke app ops assure packet did not capture the latest certification JSON."
      exit 1
    fi
    if ! find "$target_dir/ops/governance" -name 'ops-govern.json' -print -quit | grep -q .; then
      echo "External ${preset_id} smoke app ops govern JSON was not produced."
      exit 1
    fi
    if ! find "$target_dir/ops/governance" -name 'ops-govern.md' -print -quit | grep -q .; then
      echo "External ${preset_id} smoke app ops govern Markdown was not produced."
      exit 1
    fi
    if ! find "$target_dir/ops/governance" -name 'ops-govern.tgz' -print -quit | grep -q .; then
      echo "External ${preset_id} smoke app ops govern archive was not produced."
      exit 1
    fi
    if ! find "$target_dir/ops/governance" -path '*/packet/assurance/ops-assure.json' -print -quit | grep -q .; then
      echo "External ${preset_id} smoke app ops govern packet did not capture the latest assurance JSON."
      exit 1
    fi
    if ! find "$target_dir/ops/oversight" -name 'ops-oversight.json' -print -quit | grep -q .; then
      echo "External ${preset_id} smoke app ops oversight JSON was not produced."
      exit 1
    fi
    if ! find "$target_dir/ops/oversight" -name 'ops-oversight.md' -print -quit | grep -q .; then
      echo "External ${preset_id} smoke app ops oversight Markdown was not produced."
      exit 1
    fi
    if ! find "$target_dir/ops/oversight" -name 'ops-oversight.tgz' -print -quit | grep -q .; then
      echo "External ${preset_id} smoke app ops oversight archive was not produced."
      exit 1
    fi
    if ! find "$target_dir/ops/oversight" -path '*/packet/governance/ops-govern.json' -print -quit | grep -q .; then
      echo "External ${preset_id} smoke app ops oversight packet did not capture the latest governance JSON."
      exit 1
    fi
    if ! find "$target_dir/ops/control" -name 'ops-control.json' -print -quit | grep -q .; then
      echo "External ${preset_id} smoke app ops control JSON was not produced."
      exit 1
    fi
    if ! find "$target_dir/ops/control" -name 'ops-control.md' -print -quit | grep -q .; then
      echo "External ${preset_id} smoke app ops control Markdown was not produced."
      exit 1
    fi
    if ! find "$target_dir/ops/control" -name 'ops-control.tgz' -print -quit | grep -q .; then
      echo "External ${preset_id} smoke app ops control archive was not produced."
      exit 1
    fi
    if ! find "$target_dir/ops/control" -path '*/packet/oversight/ops-oversight.json' -print -quit | grep -q .; then
      echo "External ${preset_id} smoke app ops control packet did not capture the latest oversight JSON."
      exit 1
    fi
    if ! find "$target_dir/ops/authority" -name 'ops-authority.json' -print -quit | grep -q .; then
      echo "External ${preset_id} smoke app ops authority JSON was not produced."
      exit 1
    fi
    if ! find "$target_dir/ops/authority" -name 'ops-authority.md' -print -quit | grep -q .; then
      echo "External ${preset_id} smoke app ops authority Markdown was not produced."
      exit 1
    fi
    if ! find "$target_dir/ops/authority" -name 'ops-authority.tgz' -print -quit | grep -q .; then
      echo "External ${preset_id} smoke app ops authority archive was not produced."
      exit 1
    fi
    if ! find "$target_dir/ops/authority" -path '*/packet/control/ops-control.json' -print -quit | grep -q .; then
    ...
    if [ "$(find "$target_dir/ops/certifications" -mindepth 1 -maxdepth 1 -type d | wc -l | tr -d ' ')" -ne 1 ]; then
      echo "External ${preset_id} smoke app ops certification retention did not keep exactly one directory."
      exit 1
    fi
    if [ "$(find "$target_dir/ops/assurances" -mindepth 1 -maxdepth 1 -type d | wc -l | tr -d ' ')" -ne 1 ]; then
      echo "External ${preset_id} smoke app ops assurance retention did not keep exactly one directory."
      exit 1
    fi
    if [ "$(find "$target_dir/ops/governance" -mindepth 1 -maxdepth 1 -type d | wc -l | tr -d ' ')" -ne 1 ]; then
      echo "External ${preset_id} smoke app ops governance retention did not keep exactly one directory."
      exit 1
    fi
    if [ "$(find "$target_dir/ops/oversight" -mindepth 1 -maxdepth 1 -type d | wc -l | tr -d ' ')" -ne 1 ]; then
      echo "External ${preset_id} smoke app ops oversight retention did not keep exactly one directory."
      exit 1
    fi
    if [ "$(find "$target_dir/ops/control" -mindepth 1 -maxdepth 1 -type d | wc -l | tr -d ' ')" -ne 1 ]; then
      echo "External ${preset_id} smoke app ops control retention did not keep exactly one directory."
      exit 1
    fi
