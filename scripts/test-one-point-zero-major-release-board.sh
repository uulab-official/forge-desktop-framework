...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/forge-one-point-zero-major-board.XXXXXX")"
...
cat > "$WORK_DIR/one-point-zero-release-candidate.json" <<'EOF'
{
  ...
  "currentTag": "v0.1.80",
  "currentCommit": "deadbeef80",
  "nextVersion": "1.0.0",
  "nextChecklistPath": "docs/release-checklists/v1.0.0.md",
  "checklist": {
    "status": "ready",
    "summary": "turn the final first-major-release signoff into an explicit board review artifact.",
    "userFacingChange": "maintainers now get one-point-zero-major-release-board.md/json as the final board-facing review artifact before 1.0.0.",
    "followUpRisks": "the board artifact must stay aligned with signoff, checklist, and ship commands",
    "nextTarget": "use the board artifact as the last review surface before the first pnpm release:ship major"
  },
  "officialPresets": [
    { "preset": "launch-ready", "passed": true },
    { "preset": "support-ready", "passed": true },
...
[[ -f "$WORK_DIR/output/one-point-zero-major-release-board.md" ]]
[[ -f "$WORK_DIR/output/one-point-zero-major-release-board.json" ]]

node - "$WORK_DIR/output/one-point-zero-major-release-board.json" <<'NODE'
const fs = require('node:fs');

const payload = JSON.parse(fs.readFileSync(process.argv[2], 'utf8'));

if (payload.board !== 'ready-for-board-review') {
  throw new Error(`Expected board ready-for-board-review, found ${payload.board}`);
}

for (const command of ['pnpm release:major:signoff:test', 'pnpm release:ship major']) {
  if (!payload.validationCommands.includes(command) && !payload.shipCommands.includes(command)) {
    throw new Error(`Missing expected command in board artifact: ${command}`);
  }
}
NODE

echo "One Point Zero major release board smoke test passed."
