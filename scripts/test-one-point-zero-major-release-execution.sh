...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/forge-one-point-zero-major-execution.XXXXXX")"
...
cat > "$WORK_DIR/one-point-zero-release-candidate.json" <<'EOF'
{
  ...
  "currentTag": "v0.1.91",
  "currentCommit": "deadbeef91",
  "nextVersion": "1.0.0",
  "nextChecklistPath": "docs/release-checklists/v1.0.0.md",
  "checklist": {
    "status": "ready",
    "summary": "turn the final major-release activation artifact into an explicit execution confirmation surface.",
    "userFacingChange": "maintainers now get one-point-zero-major-release-execution.md/json as the final execution confirmation surface before the first 1.0.0 ship.",
    "followUpRisks": "the execution artifact must stay aligned with the activation artifact and prepared v1.0.0 checklist",
    "nextTarget": "use the execution artifact as the last execution confirmation surface before pnpm release:ship major"
  },
  "officialPresets": [
    { "preset": "launch-ready", "passed": true },
    { "preset": "support-ready", "passed": true },
...
[[ -f "$WORK_DIR/output/one-point-zero-major-release-execution.md" ]]
[[ -f "$WORK_DIR/output/one-point-zero-major-release-execution.json" ]]

node - "$WORK_DIR/output/one-point-zero-major-release-execution.json" <<'NODE'
const fs = require('node:fs');

const payload = JSON.parse(fs.readFileSync(process.argv[2], 'utf8'));

if (payload.execution !== 'ready-for-major-release-execution') {
  throw new Error(`Expected execution ready-for-major-release-execution, found ${payload.execution}`);
}

for (const command of ['pnpm release:major:activation:test', 'pnpm release:major:execution:test', 'pnpm release:ship major']) {
  if (!payload.validationCommands.includes(command) && !payload.shipCommands.includes(command)) {
    throw new Error(`Missing expected command in execution artifact: ${command}`);
  }
}
NODE

echo "One Point Zero major release execution smoke test passed."
