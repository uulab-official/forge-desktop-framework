...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/forge-one-point-zero-major-authorization.XXXXXX")"
...
cat > "$WORK_DIR/one-point-zero-release-candidate.json" <<'EOF'
{
  ...
  "currentVersion": "0.1.82",
  "currentTag": "v0.1.82",
  "currentCommit": "deadbeef82",
  "nextVersion": "1.0.0",
  "nextChecklistPath": "docs/release-checklists/v1.0.0.md",
  "checklist": {
    "status": "ready",
    "summary": "turn the final maintainer verdict artifact into an explicit major-release authorization surface.",
    "userFacingChange": "maintainers now get one-point-zero-major-release-authorization.md/json as the final execution authorization artifact before the first 1.0.0 ship.",
    "followUpRisks": "the authorization artifact must stay aligned with the verdict artifact and prepared v1.0.0 checklist",
    "nextTarget": "use the authorization artifact as the final execution handoff before pnpm release:ship major"
  },
  "officialPresets": [
    { "preset": "launch-ready", "passed": true },
    { "preset": "support-ready", "passed": true },
...
[[ -f "$WORK_DIR/output/one-point-zero-major-release-authorization.md" ]]
[[ -f "$WORK_DIR/output/one-point-zero-major-release-authorization.json" ]]

node - "$WORK_DIR/output/one-point-zero-major-release-authorization.json" <<'NODE'
const fs = require('node:fs');

const payload = JSON.parse(fs.readFileSync(process.argv[2], 'utf8'));

if (payload.authorization !== 'authorized-for-major-ship') {
  throw new Error(`Expected authorization authorized-for-major-ship, found ${payload.authorization}`);
}

for (const command of ['pnpm release:major:verdict:test', 'pnpm release:major:authorization:test', 'pnpm release:ship major']) {
  if (!payload.validationCommands.includes(command) && !payload.shipCommands.includes(command)) {
    throw new Error(`Missing expected command in authorization artifact: ${command}`);
  }
}
NODE

echo "One Point Zero major release authorization smoke test passed."
