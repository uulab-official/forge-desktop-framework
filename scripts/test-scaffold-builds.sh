...
if ! find "$PRODUCTION_READY_APP/ops/continuity" -name 'ops-continuity.json' -print -quit | grep -q .; then
  echo "Production-ready smoke app ops continuity JSON was not produced."
  exit 1
fi
if ! find "$PRODUCTION_READY_APP/ops/continuity" -name 'ops-continuity.md' -print -quit | grep -q .; then
  echo "Production-ready smoke app ops continuity Markdown was not produced."
  exit 1
fi
if ! find "$PRODUCTION_READY_APP/ops/continuity" -name 'ops-continuity.tgz' -print -quit | grep -q .; then
  echo "Production-ready smoke app ops continuity archive was not produced."
  exit 1
fi
if ! find "$PRODUCTION_READY_APP/ops/continuity" -path '*/packet/escalation/ops-escalate.json' -print -quit | grep -q .; then
  echo "Production-ready smoke app ops continuity packet did not capture the latest escalation JSON."
  exit 1
fi
if ! find "$PRODUCTION_READY_APP/ops/resilience" -name 'ops-resilience.json' -print -quit | grep -q .; then
  echo "Production-ready smoke app ops resilience JSON was not produced."
  exit 1
fi
if ! find "$PRODUCTION_READY_APP/ops/resilience" -name 'ops-resilience.md' -print -quit | grep -q .; then
  echo "Production-ready smoke app ops resilience Markdown was not produced."
  exit 1
fi
if ! find "$PRODUCTION_READY_APP/ops/resilience" -name 'ops-resilience.tgz' -print -quit | grep -q .; then
  echo "Production-ready smoke app ops resilience archive was not produced."
  exit 1
fi
if ! find "$PRODUCTION_READY_APP/ops/resilience" -path '*/packet/continuity/ops-continuity.json' -print -quit | grep -q .; then
  echo "Production-ready smoke app ops resilience packet did not capture the latest continuity JSON."
  exit 1
fi
if ! find "$PRODUCTION_READY_APP/ops/runbooks" -name 'ops-runbook.json' -print -quit | grep -q .; then
  echo "Production-ready smoke app ops runbook JSON was not produced."
  exit 1
fi
if ! find "$PRODUCTION_READY_APP/ops/runbooks" -name 'ops-runbook.md' -print -quit | grep -q .; then
  echo "Production-ready smoke app ops runbook Markdown was not produced."
  exit 1
fi
if ! find "$PRODUCTION_READY_APP/ops/runbooks" -name 'ops-runbook.tgz' -print -quit | grep -q .; then
  echo "Production-ready smoke app ops runbook archive was not produced."
  exit 1
fi
if ! find "$PRODUCTION_READY_APP/ops/runbooks" -path '*/packet/resilience/ops-resilience.json' -print -quit | grep -q .; then
  echo "Production-ready smoke app ops runbook packet did not capture the latest resilience JSON."
  exit 1
fi
if ! find "$PRODUCTION_READY_APP/ops/integrity" -name 'ops-integrity.json' -print -quit | grep -q .; then
  echo "Production-ready smoke app ops integrity JSON was not produced."
  exit 1
fi
if ! find "$PRODUCTION_READY_APP/ops/integrity" -name 'ops-integrity.md' -print -quit | grep -q .; then
  echo "Production-ready smoke app ops integrity Markdown was not produced."
  exit 1
fi
if ! find "$PRODUCTION_READY_APP/ops/integrity" -name 'ops-integrity.tgz' -print -quit | grep -q .; then
  echo "Production-ready smoke app ops integrity archive was not produced."
  exit 1
fi
if ! find "$PRODUCTION_READY_APP/ops/integrity" -path '*/packet/runbook/ops-runbook.json' -print -quit | grep -q .; then
  echo "Production-ready smoke app ops integrity packet did not capture the latest runbook JSON."
  exit 1
fi
if ! find "$PRODUCTION_READY_APP/ops/compliance" -name 'ops-compliance.json' -print -quit | grep -q .; then
  echo "Production-ready smoke app ops compliance JSON was not produced."
  exit 1
fi
if ! find "$PRODUCTION_READY_APP/ops/compliance" -name 'ops-compliance.md' -print -quit | grep -q .; then
  echo "Production-ready smoke app ops compliance Markdown was not produced."
  exit 1
fi
if ! find "$PRODUCTION_READY_APP/ops/compliance" -name 'ops-compliance.tgz' -print -quit | grep -q .; then
  echo "Production-ready smoke app ops compliance archive was not produced."
  exit 1
fi
if ! find "$PRODUCTION_READY_APP/ops/compliance" -path '*/packet/integrity/ops-integrity.json' -print -quit | grep -q .; then
...
if ! find "$PRODUCTION_READY_APP/ops/assurances" -name 'ops-assure.json' -print -quit | grep -q .; then
  echo "Production-ready smoke app ops assure JSON was not produced."
  exit 1
fi
if ! find "$PRODUCTION_READY_APP/ops/assurances" -name 'ops-assure.md' -print -quit | grep -q .; then
  echo "Production-ready smoke app ops assure Markdown was not produced."
  exit 1
fi
if ! find "$PRODUCTION_READY_APP/ops/assurances" -name 'ops-assure.tgz' -print -quit | grep -q .; then
  echo "Production-ready smoke app ops assure archive was not produced."
  exit 1
fi
if ! find "$PRODUCTION_READY_APP/ops/assurances" -path '*/packet/certification/ops-certify.json' -print -quit | grep -q .; then
  echo "Production-ready smoke app ops assure packet did not capture the latest certification JSON."
  exit 1
fi
if ! find "$PRODUCTION_READY_APP/ops/governance" -name 'ops-govern.json' -print -quit | grep -q .; then
  echo "Production-ready smoke app ops govern JSON was not produced."
  exit 1
fi
if ! find "$PRODUCTION_READY_APP/ops/governance" -name 'ops-govern.md' -print -quit | grep -q .; then
  echo "Production-ready smoke app ops govern Markdown was not produced."
  exit 1
fi
if ! find "$PRODUCTION_READY_APP/ops/governance" -name 'ops-govern.tgz' -print -quit | grep -q .; then
  echo "Production-ready smoke app ops govern archive was not produced."
  exit 1
fi
if ! find "$PRODUCTION_READY_APP/ops/governance" -path '*/packet/assurance/ops-assure.json' -print -quit | grep -q .; then
...
if ! find "$PRODUCTION_READY_APP/ops/control" -name 'ops-control.json' -print -quit | grep -q .; then
  echo "Production-ready smoke app ops control JSON was not produced."
  exit 1
fi
if ! find "$PRODUCTION_READY_APP/ops/control" -name 'ops-control.md' -print -quit | grep -q .; then
  echo "Production-ready smoke app ops control Markdown was not produced."
  exit 1
fi
if ! find "$PRODUCTION_READY_APP/ops/control" -name 'ops-control.tgz' -print -quit | grep -q .; then
  echo "Production-ready smoke app ops control archive was not produced."
  exit 1
fi
if ! find "$PRODUCTION_READY_APP/ops/control" -path '*/packet/oversight/ops-oversight.json' -print -quit | grep -q .; then
...
if ! find "$PRODUCTION_READY_APP/ops/stewardship" -name 'ops-stewardship.json' -print -quit | grep -q .; then
  echo "Production-ready smoke app ops stewardship JSON was not produced."
  exit 1
fi
if ! find "$PRODUCTION_READY_APP/ops/stewardship" -name 'ops-stewardship.md' -print -quit | grep -q .; then
  echo "Production-ready smoke app ops stewardship Markdown was not produced."
  exit 1
fi
if ! find "$PRODUCTION_READY_APP/ops/stewardship" -name 'ops-stewardship.tgz' -print -quit | grep -q .; then
  echo "Production-ready smoke app ops stewardship archive was not produced."
  exit 1
fi
if ! find "$PRODUCTION_READY_APP/ops/stewardship" -path '*/packet/authority/ops-authority.json' -print -quit | grep -q .; then
  echo "Production-ready smoke app ops stewardship packet did not capture the latest authority JSON."
  exit 1
fi
...
if [ "$(find "$PRODUCTION_READY_APP/ops/continuity" -mindepth 1 -maxdepth 1 -type d | wc -l | tr -d ' ')" -ne 1 ]; then
  echo "Production-ready smoke app ops continuity retention did not keep exactly one directory."
  exit 1
fi
if [ "$(find "$PRODUCTION_READY_APP/ops/resilience" -mindepth 1 -maxdepth 1 -type d | wc -l | tr -d ' ')" -ne 1 ]; then
  echo "Production-ready smoke app ops resilience retention did not keep exactly one directory."
  exit 1
fi
if [ "$(find "$PRODUCTION_READY_APP/ops/runbooks" -mindepth 1 -maxdepth 1 -type d | wc -l | tr -d ' ')" -ne 1 ]; then
  echo "Production-ready smoke app ops runbook retention did not keep exactly one directory."
  exit 1
fi
if [ "$(find "$PRODUCTION_READY_APP/ops/integrity" -mindepth 1 -maxdepth 1 -type d | wc -l | tr -d ' ')" -ne 1 ]; then
  echo "Production-ready smoke app ops integrity retention did not keep exactly one directory."
  exit 1
fi
...
if [ "$(find "$PRODUCTION_READY_APP/ops/assurances" -mindepth 1 -maxdepth 1 -type d | wc -l | tr -d ' ')" -ne 1 ]; then
  echo "Production-ready smoke app ops assurance retention did not keep exactly one directory."
  exit 1
fi
...
if [ "$(find "$PRODUCTION_READY_APP/ops/stewardship" -mindepth 1 -maxdepth 1 -type d | wc -l | tr -d ' ')" -ne 1 ]; then
  echo "Production-ready smoke app ops stewardship retention did not keep exactly one directory."
  exit 1
fi
