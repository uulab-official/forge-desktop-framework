...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/forge-one-point-zero-major-canon.XXXXXX")"
...
cat > "$WORK_DIR/one-point-zero-release-candidate.json" <<'EOF'
{
  ...
  "currentVersion": "0.1.95",
  "currentTag": "v0.1.95",
  "currentCommit": "deadbeef95",
  "nextVersion": "1.0.0",
  "nextChecklistPath": "docs/release-checklists/v1.0.0.md",
  "checklist": {
    "status": "ready",
    "summary": "add a final one-point-zero major-release canon artifact that turns the charter artifact plus prepared v1.0.0 checklist into the last immutable canon before the first 1.0.0 ship.",
    "userFacingChange": "maintainers now get one-point-zero-major-release-canon.md/json as the final canon surface before the first 1.0.0 ship.",
    "followUpRisks": "the canon artifact must stay aligned with the charter artifact and prepared v1.0.0 checklist",
    "nextTarget": "use the canon artifact as the last immutable canon before pnpm release:ship major"
  },
  "officialPresets": [
    { "preset": "launch-ready", "passed": true },
    { "preset": "support-ready", "passed": true },
...
[[ -f "$WORK_DIR/output/one-point-zero-major-release-canon.md" ]]
[[ -f "$WORK_DIR/output/one-point-zero-major-release-canon.json" ]]

node - "$WORK_DIR/output/one-point-zero-major-release-canon.json" <<'NODE'
const fs = require('node:fs');

const payload = JSON.parse(fs.readFileSync(process.argv[2], 'utf8'));

if (payload.canon !== 'ready-for-major-release-canon') {
  throw new Error(`Expected canon ready-for-major-release-canon, found ${payload.canon}`);
}

for (const command of ['pnpm release:major:execution:test', 'pnpm release:major:attestation:test', 'pnpm release:major:seal:test', 'pnpm release:major:charter:test', 'pnpm release:major:canon:test', 'pnpm release:ship major']) {
  if (!payload.validationCommands.includes(command) && !payload.shipCommands.includes(command)) {
    throw new Error(`Missing expected command in canon artifact: ${command}`);
  }
}
NODE

echo "One Point Zero major release canon smoke test passed."
