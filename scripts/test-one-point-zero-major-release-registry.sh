...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/forge-one-point-zero-major-registry.XXXXXX")"
...
cat > "$WORK_DIR/one-point-zero-release-candidate.json" <<'EOF'
{
  ...
  "checklist": {
    ...
    "userFacingChange": "maintainers now get one-point-zero-major-release-registry.md/json as the final registry surface before the first 1.0.0 ship.",
    "followUpRisks": "the registry artifact must stay aligned with the vault artifact and prepared v1.0.0 checklist",
    "nextTarget": "use the registry artifact as the last immutable registry before pnpm release:ship major"
  },
  "officialPresets": [
    { "preset": "launch-ready", "passed": true },
    { "preset": "support-ready", "passed": true },
...
[[ -f "$WORK_DIR/output/one-point-zero-major-release-registry.md" ]]
[[ -f "$WORK_DIR/output/one-point-zero-major-release-registry.json" ]]

node - "$WORK_DIR/output/one-point-zero-major-release-registry.json" <<'NODE'
const fs = require('node:fs');
const payload = JSON.parse(fs.readFileSync(process.argv[2], 'utf8'));

if (payload.registry !== 'ready-for-major-release-registry') {
  throw new Error(`Expected registry ready-for-major-release-registry, found ${payload.registry}`);
}
...
for (const command of [
  ...
  if (!payload.validationCommands.includes(command) && !payload.shipCommands.includes(command)) {
    throw new Error(`Missing expected command in registry artifact: ${command}`);
  }
}
NODE

echo "One Point Zero major release registry smoke test passed."
