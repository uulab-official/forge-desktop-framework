...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/forge-one-point-zero-major-attestation.XXXXXX")"
...
cat > "$WORK_DIR/one-point-zero-release-candidate.json" <<'EOF'
{
  ...
  "currentVersion": "0.1.92",
  "currentTag": "v0.1.92",
  "currentCommit": "deadbeef92",
  "nextVersion": "1.0.0",
  ...
  "checklist": {
    ...
    "userFacingChange": "maintainers now get one-point-zero-major-release-attestation.md/json as the final immutable attestation surface before the first 1.0.0 ship.",
    "followUpRisks": "the attestation artifact must stay aligned with the execution artifact and prepared v1.0.0 checklist",
    "nextTarget": "use the attestation artifact as the last immutable record before pnpm release:ship major"
  },
  "officialPresets": [
    { "preset": "launch-ready", "passed": true },
    { "preset": "support-ready", "passed": true },
...
[[ -f "$WORK_DIR/output/one-point-zero-major-release-attestation.md" ]]
[[ -f "$WORK_DIR/output/one-point-zero-major-release-attestation.json" ]]

node - "$WORK_DIR/output/one-point-zero-major-release-attestation.json" <<'NODE'
const fs = require('node:fs');

const payload = JSON.parse(fs.readFileSync(process.argv[2], 'utf8'));

if (payload.attestation !== 'ready-for-major-release-attestation') {
  throw new Error(`Expected attestation ready-for-major-release-attestation, found ${payload.attestation}`);
}

for (const command of ['pnpm release:major:activation:test', 'pnpm release:major:execution:test', 'pnpm release:major:attestation:test', 'pnpm release:ship major']) {
  if (!payload.validationCommands.includes(command) && !payload.shipCommands.includes(command)) {
    throw new Error(`Missing expected command in attestation artifact: ${command}`);
  }
}
NODE

echo "One Point Zero major release attestation smoke test passed."
