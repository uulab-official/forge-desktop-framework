...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/forge-one-point-zero-major-compendium.XXXXXX")"
...
cat > "$WORK_DIR/one-point-zero-release-candidate.json" <<'EOF'
{
  ...
  "checklist": {
    ...
    "userFacingChange": "maintainers now get one-point-zero-major-release-compendium.md/json as the final compendium surface before the first 1.0.0 ship.",
    "followUpRisks": "the compendium artifact must stay aligned with the portfolio artifact and prepared v1.0.0 checklist",
    "nextTarget": "use the compendium artifact as the last immutable compendium before pnpm release:ship major"
  },
  "officialPresets": [
    { "preset": "launch-ready", "passed": true },
    { "preset": "support-ready", "passed": true },
...
[[ -f "$WORK_DIR/output/one-point-zero-major-release-compendium.md" ]]
[[ -f "$WORK_DIR/output/one-point-zero-major-release-compendium.json" ]]

node - "$WORK_DIR/output/one-point-zero-major-release-compendium.json" <<'NODE'
const fs = require('node:fs');
const payload = JSON.parse(fs.readFileSync(process.argv[2], 'utf8'));

if (payload.compendium !== 'ready-for-major-release-compendium') {
  throw new Error(`Expected compendium ready-for-major-release-compendium, found ${payload.compendium}`);
}
...
for (const command of [
  ...
  if (!payload.validationCommands.includes(command) && !payload.shipCommands.includes(command)) {
    throw new Error(`Missing expected command in compendium artifact: ${command}`);
  }
}
NODE

echo "One Point Zero major release compendium smoke test passed."
