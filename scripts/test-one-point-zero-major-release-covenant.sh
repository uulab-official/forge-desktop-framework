...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/forge-one-point-zero-major-covenant.XXXXXX")"
...
cat > "$WORK_DIR/one-point-zero-release-candidate.json" <<'EOF'
{
  ...
  "checklist": {
    ...
    "userFacingChange": "maintainers now get one-point-zero-major-release-covenant.md/json as the final covenant surface before the first 1.0.0 ship.",
    "followUpRisks": "the covenant artifact must stay aligned with the constitution artifact and prepared v1.0.0 checklist",
    "nextTarget": "use the covenant artifact as the last immutable covenant before pnpm release:ship major"
  },
  "officialPresets": [
    { "preset": "launch-ready", "passed": true },
    { "preset": "support-ready", "passed": true },
...
[[ -f "$WORK_DIR/output/one-point-zero-major-release-covenant.md" ]]
[[ -f "$WORK_DIR/output/one-point-zero-major-release-covenant.json" ]]

node - "$WORK_DIR/output/one-point-zero-major-release-covenant.json" <<'NODE'
const fs = require('node:fs');

const payload = JSON.parse(fs.readFileSync(process.argv[2], 'utf8'));

if (payload.covenant !== 'ready-for-major-release-covenant') {
  throw new Error(`Expected covenant ready-for-major-release-covenant, found ${payload.covenant}`);
}

for (const command of ['pnpm release:major:execution:test', 'pnpm release:major:attestation:test', 'pnpm release:major:seal:test', 'pnpm release:major:charter:test', 'pnpm release:major:canon:test', 'pnpm release:major:constitution:test', 'pnpm release:major:covenant:test', 'pnpm release:ship major']) {
  if (!payload.validationCommands.includes(command) && !payload.shipCommands.includes(command)) {
    throw new Error(`Missing expected command in covenant artifact: ${command}`);
  }
}
NODE

echo "One Point Zero major release covenant smoke test passed."
