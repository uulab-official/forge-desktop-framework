...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/forge-one-point-zero-major-rehearsal.XXXXXX")"
...
cat > "$WORK_DIR/one-point-zero-release-candidate.json" <<'EOF'
{
  ...
  "currentVersion": "0.1.88",
  "currentTag": "v0.1.88",
  "currentCommit": "deadbeef88",
  "nextVersion": "1.0.0",
  "nextChecklistPath": "docs/release-checklists/v1.0.0.md",
  "checklist": {
    "status": "ready",
    "summary": "turn the final trigger artifact into an explicit major-release rehearsal surface.",
    "userFacingChange": "maintainers now get one-point-zero-major-release-rehearsal.md/json as the final dry-run surface before the first 1.0.0 ship.",
    "followUpRisks": "the rehearsal artifact must stay aligned with the trigger artifact and prepared v1.0.0 checklist",
    "nextTarget": "use the rehearsal artifact as the final dry-run layer before pnpm release:ship major"
  },
  "officialPresets": [
    { "preset": "launch-ready", "passed": true },
    { "preset": "support-ready", "passed": true },
...
[[ -f "$WORK_DIR/output/one-point-zero-major-release-rehearsal.md" ]]
[[ -f "$WORK_DIR/output/one-point-zero-major-release-rehearsal.json" ]]

node - "$WORK_DIR/output/one-point-zero-major-release-rehearsal.json" <<'NODE'
const fs = require('node:fs');

const payload = JSON.parse(fs.readFileSync(process.argv[2], 'utf8'));

if (payload.rehearsal !== 'ready-for-major-release-rehearsal') {
  throw new Error(`Expected rehearsal ready-for-major-release-rehearsal, found ${payload.rehearsal}`);
}

for (const command of ['pnpm release:major:trigger:test', 'pnpm release:major:rehearsal:test', 'pnpm release:ship major']) {
  if (!payload.validationCommands.includes(command) && !payload.shipCommands.includes(command)) {
    throw new Error(`Missing expected command in rehearsal artifact: ${command}`);
  }
}
NODE

echo "One Point Zero major release rehearsal smoke test passed."
