...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/forge-one-point-zero-major-capsule.XXXXXX")"
...
cat > "$WORK_DIR/one-point-zero-release-candidate.json" <<'EOF'
{
  ...
  "checklist": {
    ...
    "userFacingChange": "maintainers now get one-point-zero-major-release-capsule.md/json as the final capsule surface before the first 1.0.0 ship.",
    "followUpRisks": "the capsule artifact must stay aligned with the compact artifact and prepared v1.0.0 checklist",
    "nextTarget": "use the capsule artifact as the last immutable capsule before pnpm release:ship major"
  },
  "officialPresets": [
    { "preset": "launch-ready", "passed": true },
    { "preset": "support-ready", "passed": true },
...
[[ -f "$WORK_DIR/output/one-point-zero-major-release-capsule.md" ]]
[[ -f "$WORK_DIR/output/one-point-zero-major-release-capsule.json" ]]

node - "$WORK_DIR/output/one-point-zero-major-release-capsule.json" <<'NODE'
const fs = require('node:fs');

const payload = JSON.parse(fs.readFileSync(process.argv[2], 'utf8'));

if (payload.capsule !== 'ready-for-major-release-capsule') {
  throw new Error(`Expected capsule ready-for-major-release-capsule, found ${payload.capsule}`);
}

for (const command of ['pnpm release:major:execution:test', 'pnpm release:major:attestation:test', 'pnpm release:major:seal:test', 'pnpm release:major:charter:test', 'pnpm release:major:canon:test', 'pnpm release:major:constitution:test', 'pnpm release:major:covenant:test', 'pnpm release:major:compact:test', 'pnpm release:major:capsule:test', 'pnpm release:ship major']) {
  if (!payload.validationCommands.includes(command) && !payload.shipCommands.includes(command)) {
    throw new Error(`Missing expected command in capsule artifact: ${command}`);
  }
}
NODE

echo "One Point Zero major release capsule smoke test passed."
