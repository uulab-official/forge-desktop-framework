...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/forge-one-point-zero-major-abstract.XXXXXX")"
...
cat > "$WORK_DIR/one-point-zero-release-candidate.json" <<'EOF'
{
  ...
  "currentVersion": "0.1.115",
  "currentTag": "v0.1.115",
  "currentCommit": "deadbeef115",
  "nextVersion": "1.0.0",
  "nextChecklistPath": "docs/release-checklists/v1.0.0.md",
  "checklist": {
    "status": "ready",
    "summary": "add a final one-point-zero major-release abstract artifact that turns the brief artifact plus prepared v1.0.0 checklist into the last immutable abstract before the first 1.0.0 ship.",
    "userFacingChange": "maintainers now get one-point-zero-major-release-abstract.md/json as the final abstract surface before the first 1.0.0 ship.",
    "followUpRisks": "the abstract artifact must stay aligned with the brief artifact and prepared v1.0.0 checklist",
    "nextTarget": "use the abstract artifact as the last immutable abstract before pnpm release:ship major"
  },
  "officialPresets": [
    { "preset": "launch-ready", "passed": true },
    { "preset": "support-ready", "passed": true },
...
bash scripts/generate-one-point-zero-major-release-compendium.sh "$WORK_DIR/output/one-point-zero-major-release-portfolio.json" "$TARGET" "$WORK_DIR/output"
bash scripts/generate-one-point-zero-major-release-anthology.sh "$WORK_DIR/output/one-point-zero-major-release-compendium.json" "$TARGET" "$WORK_DIR/output"
bash scripts/generate-one-point-zero-major-release-omnibus.sh "$WORK_DIR/output/one-point-zero-major-release-anthology.json" "$TARGET" "$WORK_DIR/output"
bash scripts/generate-one-point-zero-major-release-digest.sh "$WORK_DIR/output/one-point-zero-major-release-omnibus.json" "$TARGET" "$WORK_DIR/output"
bash scripts/generate-one-point-zero-major-release-synopsis.sh "$WORK_DIR/output/one-point-zero-major-release-digest.json" "$TARGET" "$WORK_DIR/output"
bash scripts/generate-one-point-zero-major-release-brief.sh "$WORK_DIR/output/one-point-zero-major-release-synopsis.json" "$TARGET" "$WORK_DIR/output"
bash scripts/generate-one-point-zero-major-release-abstract.sh "$WORK_DIR/output/one-point-zero-major-release-brief.json" "$TARGET" "$WORK_DIR/output"

[[ -f "$WORK_DIR/output/one-point-zero-major-release-abstract.md" ]]
[[ -f "$WORK_DIR/output/one-point-zero-major-release-abstract.json" ]]

node - "$WORK_DIR/output/one-point-zero-major-release-abstract.json" <<'NODE'
const fs = require('node:fs');
const payload = JSON.parse(fs.readFileSync(process.argv[2], 'utf8'));

if (payload.abstract !== 'ready-for-major-release-abstract') {
  throw new Error(`Expected abstract ready-for-major-release-abstract, found ${payload.abstract}`);
}
...
for (const command of [
  ...
  if (!payload.validationCommands.includes(command) && !payload.shipCommands.includes(command)) {
    throw new Error(`Missing expected command in abstract artifact: ${command}`);
  }
}
NODE

echo "One Point Zero major release abstract smoke test passed."
