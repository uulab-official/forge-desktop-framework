...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/forge-one-point-zero-major-vault.XXXXXX")"
...
cat > "$WORK_DIR/one-point-zero-release-candidate.json" <<'EOF'
{
  ...
  "checklist": {
    ...
    "userFacingChange": "maintainers now get one-point-zero-major-release-vault.md/json as the final vault surface before the first 1.0.0 ship.",
    "followUpRisks": "the vault artifact must stay aligned with the archive artifact and prepared v1.0.0 checklist",
    "nextTarget": "use the vault artifact as the last immutable vault before pnpm release:ship major"
  },
  "officialPresets": [
    { "preset": "launch-ready", "passed": true },
    { "preset": "support-ready", "passed": true },
...
[[ -f "$WORK_DIR/output/one-point-zero-major-release-vault.md" ]]
[[ -f "$WORK_DIR/output/one-point-zero-major-release-vault.json" ]]

node - "$WORK_DIR/output/one-point-zero-major-release-vault.json" <<'NODE'
const fs = require('node:fs');
const payload = JSON.parse(fs.readFileSync(process.argv[2], 'utf8'));

if (payload.vault !== 'ready-for-major-release-vault') {
  throw new Error(`Expected vault ready-for-major-release-vault, found ${payload.vault}`);
}

for (const command of ['pnpm release:major:execution:test', 'pnpm release:major:attestation:test', 'pnpm release:major:seal:test', 'pnpm release:major:charter:test', 'pnpm release:major:canon:test', 'pnpm release:major:constitution:test', 'pnpm release:major:covenant:test', 'pnpm release:major:compact:test', 'pnpm release:major:capsule:test', 'pnpm release:major:ledger:test', 'pnpm release:major:archive:test', 'pnpm release:major:vault:test', 'pnpm release:ship major']) {
  if (!payload.validationCommands.includes(command) && !payload.shipCommands.includes(command)) {
    throw new Error(`Missing expected command in vault artifact: ${command}`);
  }
}
NODE

echo "One Point Zero major release vault smoke test passed."
