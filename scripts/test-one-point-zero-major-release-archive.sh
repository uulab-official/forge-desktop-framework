...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/forge-one-point-zero-major-archive.XXXXXX")"
...
cat > "$WORK_DIR/one-point-zero-release-candidate.json" <<'EOF'
{
  ...
  "checklist": {
    ...
    "userFacingChange": "maintainers now get one-point-zero-major-release-archive.md/json as the final archive surface before the first 1.0.0 ship.",
    "followUpRisks": "the archive artifact must stay aligned with the ledger artifact and prepared v1.0.0 checklist",
    "nextTarget": "use the archive artifact as the last immutable archive before pnpm release:ship major"
  },
  "officialPresets": [
    { "preset": "launch-ready", "passed": true },
    { "preset": "support-ready", "passed": true },
...
bash scripts/generate-one-point-zero-major-release-canon.sh \
  "$WORK_DIR/output/one-point-zero-major-release-charter.json" \
  "$TARGET" \
  "$WORK_DIR/output"

bash scripts/generate-one-point-zero-major-release-constitution.sh \
  "$WORK_DIR/output/one-point-zero-major-release-canon.json" \
  "$TARGET" \
  "$WORK_DIR/output"

bash scripts/generate-one-point-zero-major-release-covenant.sh \
  "$WORK_DIR/output/one-point-zero-major-release-constitution.json" \
  "$TARGET" \
  "$WORK_DIR/output"

bash scripts/generate-one-point-zero-major-release-compact.sh \
  "$WORK_DIR/output/one-point-zero-major-release-covenant.json" \
  "$TARGET" \
  "$WORK_DIR/output"

bash scripts/generate-one-point-zero-major-release-capsule.sh \
  "$WORK_DIR/output/one-point-zero-major-release-compact.json" \
  "$TARGET" \
  "$WORK_DIR/output"

bash scripts/generate-one-point-zero-major-release-ledger.sh \
  "$WORK_DIR/output/one-point-zero-major-release-capsule.json" \
  "$TARGET" \
  "$WORK_DIR/output"

bash scripts/generate-one-point-zero-major-release-archive.sh \
  "$WORK_DIR/output/one-point-zero-major-release-ledger.json" \
  "$TARGET" \
  "$WORK_DIR/output"

[[ -f "$WORK_DIR/output/one-point-zero-major-release-archive.md" ]]
[[ -f "$WORK_DIR/output/one-point-zero-major-release-archive.json" ]]

node - "$WORK_DIR/output/one-point-zero-major-release-archive.json" <<'NODE'
const fs = require('node:fs');

const payload = JSON.parse(fs.readFileSync(process.argv[2], 'utf8'));

if (payload.archive !== 'ready-for-major-release-archive') {
  throw new Error(`Expected archive ready-for-major-release-archive, found ${payload.archive}`);
}

for (const command of ['pnpm release:major:execution:test', 'pnpm release:major:attestation:test', 'pnpm release:major:seal:test', 'pnpm release:major:charter:test', 'pnpm release:major:canon:test', 'pnpm release:major:constitution:test', 'pnpm release:major:covenant:test', 'pnpm release:major:compact:test', 'pnpm release:major:capsule:test', 'pnpm release:major:ledger:test', 'pnpm release:major:archive:test', 'pnpm release:ship major']) {
  if (!payload.validationCommands.includes(command) && !payload.shipCommands.includes(command)) {
    throw new Error(`Missing expected command in archive artifact: ${command}`);
  }
}
NODE

echo "One Point Zero major release archive smoke test passed."
