...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/forge-one-point-zero-major-go-live.XXXXXX")"
...
cat > "$WORK_DIR/one-point-zero-release-candidate.json" <<'EOF'
{
  ...
  "currentVersion": "0.1.89",
  "currentTag": "v0.1.89",
  "currentCommit": "deadbeef89",
  "nextVersion": "1.0.0",
  "nextChecklistPath": "docs/release-checklists/v1.0.0.md",
  "checklist": {
    "status": "ready",
    "summary": "turn the final rehearsal artifact into an explicit major-release go-live surface.",
    "userFacingChange": "maintainers now get one-point-zero-major-release-go-live.md/json as the final go-live surface before the first 1.0.0 ship.",
    "followUpRisks": "the go-live artifact must stay aligned with the rehearsal artifact and prepared v1.0.0 checklist",
    "nextTarget": "use the go-live artifact as the final execution surface before pnpm release:ship major"
  },
  "officialPresets": [
    { "preset": "launch-ready", "passed": true },
    { "preset": "support-ready", "passed": true },
...
[[ -f "$WORK_DIR/output/one-point-zero-major-release-go-live.md" ]]
[[ -f "$WORK_DIR/output/one-point-zero-major-release-go-live.json" ]]

node - "$WORK_DIR/output/one-point-zero-major-release-go-live.json" <<'NODE'
const fs = require('node:fs');

const payload = JSON.parse(fs.readFileSync(process.argv[2], 'utf8'));

if (payload.goLive !== 'ready-for-major-release-go-live') {
  throw new Error(`Expected goLive ready-for-major-release-go-live, found ${payload.goLive}`);
}

for (const command of ['pnpm release:major:rehearsal:test', 'pnpm release:major:go-live:test', 'pnpm release:ship major']) {
  if (!payload.validationCommands.includes(command) && !payload.shipCommands.includes(command)) {
    throw new Error(`Missing expected command in go-live artifact: ${command}`);
  }
}
NODE

echo "One Point Zero major release go-live smoke test passed."
