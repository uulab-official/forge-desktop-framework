...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/forge-one-point-zero-major-trigger.XXXXXX")"
...
cat > "$WORK_DIR/one-point-zero-release-candidate.json" <<'EOF'
{
  ...
  "currentTag": "v0.1.87",
  "currentCommit": "deadbeef87",
  "nextVersion": "1.0.0",
  "nextChecklistPath": "docs/release-checklists/v1.0.0.md",
  "checklist": {
    "status": "ready",
    "summary": "turn the final preflight artifact into an explicit major-release trigger surface.",
    "userFacingChange": "maintainers now get one-point-zero-major-release-trigger.md/json as the final execution trigger before the first 1.0.0 ship.",
    "followUpRisks": "the trigger artifact must stay aligned with the preflight artifact and prepared v1.0.0 checklist",
    "nextTarget": "use the trigger artifact as the final execution trigger before pnpm release:ship major"
  },
  "officialPresets": [
    { "preset": "launch-ready", "passed": true },
    { "preset": "support-ready", "passed": true },
...
[[ -f "$WORK_DIR/output/one-point-zero-major-release-trigger.md" ]]
[[ -f "$WORK_DIR/output/one-point-zero-major-release-trigger.json" ]]

node - "$WORK_DIR/output/one-point-zero-major-release-trigger.json" <<'NODE'
const fs = require('node:fs');

const payload = JSON.parse(fs.readFileSync(process.argv[2], 'utf8'));

if (payload.trigger !== 'ready-for-major-release-trigger') {
  throw new Error(`Expected trigger ready-for-major-release-trigger, found ${payload.trigger}`);
}

for (const command of ['pnpm release:major:preflight:test', 'pnpm release:major:trigger:test', 'pnpm release:ship major']) {
  if (!payload.validationCommands.includes(command) && !payload.shipCommands.includes(command)) {
    throw new Error(`Missing expected command in trigger artifact: ${command}`);
  }
}
NODE

echo "One Point Zero major release trigger smoke test passed."
