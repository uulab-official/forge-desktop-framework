...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/forge-one-point-zero-major-omnibus.XXXXXX")"
...
cat > "$WORK_DIR/one-point-zero-release-candidate.json" <<'EOF'
{
  ...
  "checklist": {
    ...
    "userFacingChange": "maintainers now get one-point-zero-major-release-omnibus.md/json as the final omnibus surface before the first 1.0.0 ship.",
    "followUpRisks": "the omnibus artifact must stay aligned with the anthology artifact and prepared v1.0.0 checklist",
    "nextTarget": "use the omnibus artifact as the last immutable omnibus before pnpm release:ship major"
  },
  "officialPresets": [
    { "preset": "launch-ready", "passed": true },
    { "preset": "support-ready", "passed": true },
...
[[ -f "$WORK_DIR/output/one-point-zero-major-release-omnibus.md" ]]
[[ -f "$WORK_DIR/output/one-point-zero-major-release-omnibus.json" ]]

node - "$WORK_DIR/output/one-point-zero-major-release-omnibus.json" <<'NODE'
const fs = require('node:fs');
const payload = JSON.parse(fs.readFileSync(process.argv[2], 'utf8'));

if (payload.omnibus !== 'ready-for-major-release-omnibus') {
  throw new Error(`Expected omnibus ready-for-major-release-omnibus, found ${payload.omnibus}`);
}
...
for (const command of [
  ...
  if (!payload.validationCommands.includes(command) && !payload.shipCommands.includes(command)) {
    throw new Error(`Missing expected command in omnibus artifact: ${command}`);
  }
}
NODE

echo "One Point Zero major release omnibus smoke test passed."
