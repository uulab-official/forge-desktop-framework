...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/forge-one-point-zero-major-manifest.XXXXXX")"
...
cat > "$WORK_DIR/one-point-zero-release-candidate.json" <<'EOF'
{
  ...
  "checklist": {
    ...
    "userFacingChange": "maintainers now get one-point-zero-major-release-manifest.md/json as the final manifest surface before the first 1.0.0 ship.",
    "followUpRisks": "the manifest artifact must stay aligned with the directory artifact and prepared v1.0.0 checklist",
    "nextTarget": "use the manifest artifact as the last immutable manifest before pnpm release:ship major"
  },
  "officialPresets": [
    { "preset": "launch-ready", "passed": true },
    { "preset": "support-ready", "passed": true },
...
[[ -f "$WORK_DIR/output/one-point-zero-major-release-manifest.md" ]]
[[ -f "$WORK_DIR/output/one-point-zero-major-release-manifest.json" ]]

node - "$WORK_DIR/output/one-point-zero-major-release-manifest.json" <<'NODE'
const fs = require('node:fs');
const payload = JSON.parse(fs.readFileSync(process.argv[2], 'utf8'));

if (payload.manifest !== 'ready-for-major-release-manifest') {
  throw new Error(`Expected manifest ready-for-major-release-manifest, found ${payload.manifest}`);
}
...
for (const command of [
  ...
  if (!payload.validationCommands.includes(command) && !payload.shipCommands.includes(command)) {
    throw new Error(`Missing expected command in manifest artifact: ${command}`);
  }
}
NODE

echo "One Point Zero major release manifest smoke test passed."
