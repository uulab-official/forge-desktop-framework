...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/forge-one-point-zero-major-dossier.XXXXXX")"
...
cat > "$WORK_DIR/one-point-zero-release-candidate.json" <<'EOF'
{
  ...
  "currentTag": "v0.1.106",
  "currentCommit": "deadbeef106",
  "nextVersion": "1.0.0",
  "nextChecklistPath": "docs/release-checklists/v1.0.0.md",
  "checklist": {
    "status": "ready",
    "summary": "add a final one-point-zero major-release dossier artifact that turns the manifest artifact plus prepared v1.0.0 checklist into the last immutable dossier before the first 1.0.0 ship.",
    "userFacingChange": "maintainers now get one-point-zero-major-release-dossier.md/json as the final dossier surface before the first 1.0.0 ship.",
    "followUpRisks": "the dossier artifact must stay aligned with the manifest artifact and prepared v1.0.0 checklist",
    "nextTarget": "use the dossier artifact as the last immutable dossier before pnpm release:ship major"
  },
  "officialPresets": [
    { "preset": "launch-ready", "passed": true },
    { "preset": "support-ready", "passed": true },
...
[[ -f "$WORK_DIR/output/one-point-zero-major-release-dossier.md" ]]
[[ -f "$WORK_DIR/output/one-point-zero-major-release-dossier.json" ]]

node - "$WORK_DIR/output/one-point-zero-major-release-dossier.json" <<'NODE'
const fs = require('node:fs');
const payload = JSON.parse(fs.readFileSync(process.argv[2], 'utf8'));

if (payload.dossier !== 'ready-for-major-release-dossier') {
  throw new Error(`Expected dossier ready-for-major-release-dossier, found ${payload.dossier}`);
}
...
for (const command of [
  ...
  if (!payload.validationCommands.includes(command) && !payload.shipCommands.includes(command)) {
    throw new Error(`Missing expected command in dossier artifact: ${command}`);
  }
}
NODE

echo "One Point Zero major release dossier smoke test passed."
