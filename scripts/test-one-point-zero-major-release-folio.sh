...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/forge-one-point-zero-major-folio.XXXXXX")"
...
cat > "$WORK_DIR/one-point-zero-release-candidate.json" <<'EOF'
{
  ...
  "checklist": {
    ...
    "userFacingChange": "maintainers now get one-point-zero-major-release-folio.md/json as the final folio surface before the first 1.0.0 ship.",
    "followUpRisks": "the folio artifact must stay aligned with the dossier artifact and prepared v1.0.0 checklist",
    "nextTarget": "use the folio artifact as the last immutable folio before pnpm release:ship major"
  },
  "officialPresets": [
    { "preset": "launch-ready", "passed": true },
    { "preset": "support-ready", "passed": true },
...
[[ -f "$WORK_DIR/output/one-point-zero-major-release-folio.md" ]]
[[ -f "$WORK_DIR/output/one-point-zero-major-release-folio.json" ]]

node - "$WORK_DIR/output/one-point-zero-major-release-folio.json" <<'NODE'
const fs = require('node:fs');
const payload = JSON.parse(fs.readFileSync(process.argv[2], 'utf8'));

if (payload.folio !== 'ready-for-major-release-folio') {
  throw new Error(`Expected folio ready-for-major-release-folio, found ${payload.folio}`);
}
...
for (const command of [
  ...
  if (!payload.validationCommands.includes(command) && !payload.shipCommands.includes(command)) {
    throw new Error(`Missing expected command in folio artifact: ${command}`);
  }
}
NODE

echo "One Point Zero major release folio smoke test passed."
