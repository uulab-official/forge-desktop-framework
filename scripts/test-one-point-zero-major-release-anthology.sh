...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/forge-one-point-zero-major-anthology.XXXXXX")"
...
cat > "$WORK_DIR/one-point-zero-release-candidate.json" <<'EOF'
{
  ...
  "checklist": {
    ...
    "userFacingChange": "maintainers now get one-point-zero-major-release-anthology.md/json as the final anthology surface before the first 1.0.0 ship.",
    "followUpRisks": "the anthology artifact must stay aligned with the compendium artifact and prepared v1.0.0 checklist",
    "nextTarget": "use the anthology artifact as the last immutable anthology before pnpm release:ship major"
  },
  "officialPresets": [
    { "preset": "launch-ready", "passed": true },
    { "preset": "support-ready", "passed": true },
...
[[ -f "$WORK_DIR/output/one-point-zero-major-release-anthology.md" ]]
[[ -f "$WORK_DIR/output/one-point-zero-major-release-anthology.json" ]]

node - "$WORK_DIR/output/one-point-zero-major-release-anthology.json" <<'NODE'
const fs = require('node:fs');
const payload = JSON.parse(fs.readFileSync(process.argv[2], 'utf8'));

if (payload.anthology !== 'ready-for-major-release-anthology') {
  throw new Error(`Expected anthology ready-for-major-release-anthology, found ${payload.anthology}`);
}
...
for (const command of [
  ...
  if (!payload.validationCommands.includes(command) && !payload.shipCommands.includes(command)) {
    throw new Error(`Missing expected command in anthology artifact: ${command}`);
  }
}
NODE

echo "One Point Zero major release anthology smoke test passed."
