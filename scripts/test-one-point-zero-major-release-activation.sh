...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/forge-one-point-zero-major-activation.XXXXXX")"
...
cat > "$WORK_DIR/one-point-zero-release-candidate.json" <<'EOF'
{
  ...
  "currentTag": "v0.1.90",
  "currentCommit": "deadbeef90",
  "nextVersion": "1.0.0",
  "nextChecklistPath": "docs/release-checklists/v1.0.0.md",
  "checklist": {
    "status": "ready",
    "summary": "turn the final major-release go-live artifact into an explicit activation surface.",
    "userFacingChange": "maintainers now get one-point-zero-major-release-activation.md/json as the final activation surface before the first 1.0.0 ship.",
    "followUpRisks": "the activation artifact must stay aligned with the go-live artifact and prepared v1.0.0 checklist",
    "nextTarget": "use the activation artifact as the final execution confirmation surface before pnpm release:ship major"
  },
  "officialPresets": [
    { "preset": "launch-ready", "passed": true },
    { "preset": "support-ready", "passed": true },
...
[[ -f "$WORK_DIR/output/one-point-zero-major-release-activation.md" ]]
[[ -f "$WORK_DIR/output/one-point-zero-major-release-activation.json" ]]

node - "$WORK_DIR/output/one-point-zero-major-release-activation.json" <<'NODE'
const fs = require('node:fs');

const payload = JSON.parse(fs.readFileSync(process.argv[2], 'utf8'));

if (payload.activation !== 'ready-for-major-release-activation') {
  throw new Error(`Expected activation ready-for-major-release-activation, found ${payload.activation}`);
}

for (const command of ['pnpm release:major:go-live:test', 'pnpm release:major:activation:test', 'pnpm release:ship major']) {
  if (!payload.validationCommands.includes(command) && !payload.shipCommands.includes(command)) {
    throw new Error(`Missing expected command in activation artifact: ${command}`);
  }
}
NODE

echo "One Point Zero major release activation smoke test passed."
