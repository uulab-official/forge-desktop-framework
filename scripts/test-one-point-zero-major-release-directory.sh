...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/forge-one-point-zero-major-directory.XXXXXX")"
...
cat > "$WORK_DIR/one-point-zero-release-candidate.json" <<'EOF'
{
  ...
  "currentVersion": "0.1.104",
  "currentTag": "v0.1.104",
  "currentCommit": "deadbeef104",
  "nextVersion": "1.0.0",
  ...
  "checklist": {
    ...
    "userFacingChange": "maintainers now get one-point-zero-major-release-directory.md/json as the final directory surface before the first 1.0.0 ship.",
    "followUpRisks": "the directory artifact must stay aligned with the registry artifact and prepared v1.0.0 checklist",
    "nextTarget": "use the directory artifact as the last immutable directory before pnpm release:ship major"
  },
  "officialPresets": [
    { "preset": "launch-ready", "passed": true },
    { "preset": "support-ready", "passed": true },
...
[[ -f "$WORK_DIR/output/one-point-zero-major-release-directory.md" ]]
[[ -f "$WORK_DIR/output/one-point-zero-major-release-directory.json" ]]

node - "$WORK_DIR/output/one-point-zero-major-release-directory.json" <<'NODE'
const fs = require('node:fs');
const payload = JSON.parse(fs.readFileSync(process.argv[2], 'utf8'));

if (payload.directory !== 'ready-for-major-release-directory') {
  throw new Error(`Expected directory ready-for-major-release-directory, found ${payload.directory}`);
}
...
for (const command of [
  ...
  if (!payload.validationCommands.includes(command) && !payload.shipCommands.includes(command)) {
    throw new Error(`Missing expected command in directory artifact: ${command}`);
  }
}
NODE

echo "One Point Zero major release directory smoke test passed."
