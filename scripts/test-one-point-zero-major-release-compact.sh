...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/forge-one-point-zero-major-compact.XXXXXX")"
...
cat > "$WORK_DIR/one-point-zero-release-candidate.json" <<'EOF'
{
  ...
  "checklist": {
    ...
    "userFacingChange": "maintainers now get one-point-zero-major-release-compact.md/json as the final compact surface before the first 1.0.0 ship.",
    "followUpRisks": "the compact artifact must stay aligned with the covenant artifact and prepared v1.0.0 checklist",
    "nextTarget": "use the compact artifact as the last immutable compact before pnpm release:ship major"
  },
  "officialPresets": [
    { "preset": "launch-ready", "passed": true },
    { "preset": "support-ready", "passed": true },
...
[[ -f "$WORK_DIR/output/one-point-zero-major-release-compact.md" ]]
[[ -f "$WORK_DIR/output/one-point-zero-major-release-compact.json" ]]

node - "$WORK_DIR/output/one-point-zero-major-release-compact.json" <<'NODE'
const fs = require('node:fs');

const payload = JSON.parse(fs.readFileSync(process.argv[2], 'utf8'));

if (payload.compact !== 'ready-for-major-release-compact') {
  throw new Error(`Expected compact ready-for-major-release-compact, found ${payload.compact}`);
}

for (const command of ['pnpm release:major:execution:test', 'pnpm release:major:attestation:test', 'pnpm release:major:seal:test', 'pnpm release:major:charter:test', 'pnpm release:major:canon:test', 'pnpm release:major:constitution:test', 'pnpm release:major:covenant:test', 'pnpm release:major:compact:test', 'pnpm release:ship major']) {
  if (!payload.validationCommands.includes(command) && !payload.shipCommands.includes(command)) {
    throw new Error(`Missing expected command in compact artifact: ${command}`);
  }
}
NODE

echo "One Point Zero major release compact smoke test passed."
