...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/forge-one-point-zero-major-brief.XXXXXX")"
...
cat > "$WORK_DIR/one-point-zero-release-candidate.json" <<'EOF'
{
  ...
  "currentVersion": "0.1.114",
  "currentTag": "v0.1.114",
  "currentCommit": "deadbeef114",
  "nextVersion": "1.0.0",
  ...
  "checklist": {
    ...
    "userFacingChange": "maintainers now get one-point-zero-major-release-brief.md/json as the final brief surface before the first 1.0.0 ship.",
    "followUpRisks": "the brief artifact must stay aligned with the synopsis artifact and prepared v1.0.0 checklist",
    "nextTarget": "use the brief artifact as the last immutable brief before pnpm release:ship major"
  },
  "officialPresets": [
    { "preset": "launch-ready", "passed": true },
    { "preset": "support-ready", "passed": true },
...
[[ -f "$WORK_DIR/output/one-point-zero-major-release-brief.md" ]]
[[ -f "$WORK_DIR/output/one-point-zero-major-release-brief.json" ]]

node - "$WORK_DIR/output/one-point-zero-major-release-brief.json" <<'NODE'
const fs = require('node:fs');
const payload = JSON.parse(fs.readFileSync(process.argv[2], 'utf8'));

if (payload.brief !== 'ready-for-major-release-brief') {
  throw new Error(`Expected brief ready-for-major-release-brief, found ${payload.brief}`);
}
...
for (const command of [
  ...
  if (!payload.validationCommands.includes(command) && !payload.shipCommands.includes(command)) {
    throw new Error(`Missing expected command in brief artifact: ${command}`);
  }
}
NODE

echo "One Point Zero major release brief smoke test passed."
