...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/forge-one-point-zero-major-packet.XXXXXX")"
...
cat > "$WORK_DIR/one-point-zero-release-candidate.json" <<'EOF'
{
  ...
  "currentVersion": "0.1.78",
  "currentTag": "v0.1.78",
  "currentCommit": "deadbeef78",
  "nextVersion": "1.0.0",
  "nextChecklistPath": "docs/release-checklists/v1.0.0.md",
  "checklist": {
    "status": "ready",
    "summary": "turn the final first-major-release stack into one human sign-off packet.",
    "userFacingChange": "maintainers now get one-point-zero-major-release-packet.md/json as the final human approval packet before 1.0.0.",
    "followUpRisks": "the packet must stay aligned with cockpit, checklist, and major ship commands",
    "nextTarget": "use the packet as the last sign-off surface before the first pnpm release:ship major"
  },
  "officialPresets": [
    { "preset": "launch-ready", "passed": true },
    { "preset": "support-ready", "passed": true },
...
[[ -f "$WORK_DIR/output/one-point-zero-major-release-packet.md" ]]
[[ -f "$WORK_DIR/output/one-point-zero-major-release-packet.json" ]]

node - "$WORK_DIR/output/one-point-zero-major-release-packet.json" <<'NODE'
const fs = require('node:fs');

const payload = JSON.parse(fs.readFileSync(process.argv[2], 'utf8'));

if (payload.packet !== 'ready-for-human-signoff') {
  throw new Error(`Expected packet ready-for-human-signoff, found ${payload.packet}`);
}

for (const command of ['pnpm release:major:cockpit:test', 'pnpm release:ship major']) {
  if (!payload.validationCommands.includes(command) && !payload.shipCommands.includes(command)) {
    throw new Error(`Missing expected command in packet artifact: ${command}`);
  }
}
NODE

echo "One Point Zero major release packet smoke test passed."
