...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/forge-one-point-zero-major-constitution.XXXXXX")"
...
cat > "$WORK_DIR/one-point-zero-release-candidate.json" <<'EOF'
{
  ...
  "checklist": {
    ...
    "userFacingChange": "maintainers now get one-point-zero-major-release-constitution.md/json as the final constitution surface before the first 1.0.0 ship.",
    "followUpRisks": "the constitution artifact must stay aligned with the canon artifact and prepared v1.0.0 checklist",
    "nextTarget": "use the constitution artifact as the last immutable constitution before pnpm release:ship major"
  },
  "officialPresets": [
    { "preset": "launch-ready", "passed": true },
    { "preset": "support-ready", "passed": true },
...
[[ -f "$WORK_DIR/output/one-point-zero-major-release-constitution.md" ]]
[[ -f "$WORK_DIR/output/one-point-zero-major-release-constitution.json" ]]

node - "$WORK_DIR/output/one-point-zero-major-release-constitution.json" <<'NODE'
const fs = require('node:fs');

const payload = JSON.parse(fs.readFileSync(process.argv[2], 'utf8'));

if (payload.constitution !== 'ready-for-major-release-constitution') {
  throw new Error(`Expected constitution ready-for-major-release-constitution, found ${payload.constitution}`);
}

for (const command of ['pnpm release:major:execution:test', 'pnpm release:major:attestation:test', 'pnpm release:major:seal:test', 'pnpm release:major:charter:test', 'pnpm release:major:canon:test', 'pnpm release:major:constitution:test', 'pnpm release:ship major']) {
  if (!payload.validationCommands.includes(command) && !payload.shipCommands.includes(command)) {
    throw new Error(`Missing expected command in constitution artifact: ${command}`);
  }
}
NODE

echo "One Point Zero major release constitution smoke test passed."
