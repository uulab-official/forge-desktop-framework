...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/forge-one-point-zero-major-digest.XXXXXX")"
...
cat > "$WORK_DIR/one-point-zero-release-candidate.json" <<'EOF'
{
  ...
  "currentVersion": "0.1.112",
  "currentTag": "v0.1.112",
  "currentCommit": "deadbeef112",
  "nextVersion": "1.0.0",
  ...
  "checklist": {
    ...
    "userFacingChange": "maintainers now get one-point-zero-major-release-digest.md/json as the final digest surface before the first 1.0.0 ship.",
    "followUpRisks": "the digest artifact must stay aligned with the omnibus artifact and prepared v1.0.0 checklist",
    "nextTarget": "use the digest artifact as the last immutable digest before pnpm release:ship major"
  },
  "officialPresets": [
    { "preset": "launch-ready", "passed": true },
    { "preset": "support-ready", "passed": true },
...
[[ -f "$WORK_DIR/output/one-point-zero-major-release-digest.md" ]]
[[ -f "$WORK_DIR/output/one-point-zero-major-release-digest.json" ]]

node - "$WORK_DIR/output/one-point-zero-major-release-digest.json" <<'NODE'
const fs = require('node:fs');
const payload = JSON.parse(fs.readFileSync(process.argv[2], 'utf8'));

if (payload.digest !== 'ready-for-major-release-digest') {
  throw new Error(`Expected digest ready-for-major-release-digest, found ${payload.digest}`);
}
...
for (const command of [
  ...
  if (!payload.validationCommands.includes(command) && !payload.shipCommands.includes(command)) {
    throw new Error(`Missing expected command in digest artifact: ${command}`);
  }
}
NODE

echo "One Point Zero major release digest smoke test passed."
