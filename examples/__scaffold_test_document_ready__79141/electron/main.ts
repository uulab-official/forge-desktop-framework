import { app, BrowserWindow, ipcMain, Menu, type MenuItemConstructorOptions, dialog, shell, type OpenDialogOptions } from 'electron';
...
const enabledFeatures = ["file-association","file-dialogs","recent-files","windowing","menu-bar"];
...
function buildApplicationMenu() {
  const macMenu: MenuItemConstructorOptions[] = process.platform === 'darwin'
    ? [{
        label: "Scaffold Test Document Ready 79141",
        submenu: [
          { role: 'about' as const },
  ...
  const helpMenu: MenuItemConstructorOptions = {
    ...
    submenu: [
      {
        label: 'About Scaffold Test Document Ready 79141',
        click: () => {
          if (mainWindow) {
            mainWindow.show();
            mainWindow.focus();
...
function matchesAssociatedFile(value: string | null | undefined) {
  ...
  return normalized?.toLowerCase().endsWith(".scaffoldtestdocumentready79141doc") ?? false;
}

function getFileAssociationState() {
  return {
    extension: "scaffoldtestdocumentready79141doc",
...
const fileDialogState: FileDialogState = {
  suggestedName: "scaffoldtestdocumentready79141-document.txt",
  lastOpenPath: null,
  lastSavePath: null,
  lastRevealPath: null,
  lastAction: null,
