...
async function getDiagnosticsSummary() {
  return {
    productName: "Stewardship Repro 28934",
    appId: "com.forge.stewardshiprepro28934",
...
function createDiagnosticsFileName() {
  ...
  return `stewardshiprepro28934-diagnostics-${stamp}.json`;
}

async function exportDiagnosticsBundle() {
  const supportDir = path.join(app.getPath('downloads'), "stewardshiprepro28934-support");
...
const logArchiveState: LogArchiveState = {
  logsPath: app.getPath('logs'),
  archiveDirectoryPath: path.join(app.getPath('downloads'), "stewardshiprepro28934-support", 'log-archives'),
  fileCount: 0,
  totalBytes: 0,
  files: [],
...
function createLogArchiveFolderName() {
  const stamp = new Date().toISOString().replace(/[:.]/g, '-');
  return `stewardshiprepro28934-logs-${stamp}`;
}
...
const incidentReportState: IncidentReportState = {
  directoryPath: path.join(app.getPath('downloads'), "stewardshiprepro28934-support", 'incident-reports'),
  lastExportPath: null,
...
function createIncidentReportFileName() {
  const stamp = new Date().toISOString().replace(/[:.]/g, '-');
  return `stewardshiprepro28934-incident-report-${stamp}.json`;
}
...
const diagnosticsTimelineState: DiagnosticsTimelineState = {
  directoryPath: path.join(app.getPath('downloads'), "stewardshiprepro28934-support", 'diagnostics-timeline'),
  lastExportPath: null,
  lastExportedAt: null,
  eventCount: 0,
  lastEventAt: null,
...
function createDiagnosticsTimelineFileName() {
  const stamp = new Date().toISOString().replace(/[:.]/g, '-');
  return `stewardshiprepro28934-diagnostics-timeline-${stamp}.json`;
}
...
const supportBundleState: SupportBundleState = {
  directoryPath: path.join(app.getPath('downloads'), "stewardshiprepro28934-support"),
  lastExportPath: null,
  lastGeneratedAt: null,
  lastSizeBytes: null,
  exportCount: 0,
...
function createSupportBundleFileName() {
  const stamp = new Date().toISOString().replace(/[:.]/g, '-');
  return `stewardshiprepro28934-support-bundle-${stamp}.json`;
}
...
function buildApplicationMenu() {
  ...
  const helpMenu: MenuItemConstructorOptions = {
    ...
    submenu: [
      {
        label: 'About Stewardship Repro 28934',
        click: () => {
          if (mainWindow) {
            mainWindow.show();
            mainWindow.focus();
...
function matchesAssociatedFile(value: string | null | undefined) {
  ...
  return normalized?.toLowerCase().endsWith(".stewardshiprepro28934doc") ?? false;
}

function getFileAssociationState() {
  return {
    extension: "stewardshiprepro28934doc",
...
const fileDialogState: FileDialogState = {
  suggestedName: "stewardshiprepro28934-document.txt",
  lastOpenPath: null,
  lastSavePath: null,
  lastRevealPath: null,
  lastAction: null,
...
function registerIpcHandlers() {
  ...
  ipcMain.handle(IPC_CHANNELS.NOTIFY_SHOW, async (_event, title: string, body: string) => {
    const safeTitle = title.trim() || "Stewardship Repro 28934";
