...
if (authority) {
  ...
  addCheck('index-recency-vs-authority', !index || Date.parse(index.modifiedAt) >= Date.parse(authority.modifiedAt), !index ? 'index missing' : Date.parse(index.modifiedAt) >= Date.parse(authority.modifiedAt) ? 'index is newer than authority' : 'index is older than authority', checks);
}
