...
### Production Stewardship Packets

- added generated `pnpm ops:stewardship` plus `scripts/ops-stewardship.sh`, so scaffolded `production-ready` apps now turn the latest authority packet, control packet, oversight chain, governance chain, assurance chain, release evidence, docs, env template, and packaged release output into one final Markdown, JSON, and tarball stewardship packet under `ops/stewardship/`
- updated generated `production:check`, validate workflow, tagged release workflow, `ops:index`, and retention so scaffolded apps now inventory, upload, and keep `ops/stewardship/` bounded as part of the standard production audit flow
- extended release-surface audit coverage plus internal and external `production-ready` scaffold smoke so Forge now proves the final stewardship packet exists, captures the latest authority packet, and stays coherent with release output before the framework version can move
...
- added generated `pnpm ops:certify` plus `scripts/ops-certify.sh`, so scaffolded `production-ready` apps now turn the latest compliance packet, integrity verdict, gate, attestation, release pack, export, index, operator docs, and packaged release output into one final Markdown, JSON, and tarball certification packet under `ops/certifications/`
- updated generated `production:check`, validate workflow, tagged release workflow, `ops:index`, and retention so scaffolded apps now inventory, upload, and keep `ops/certifications/` bounded as part of the standard production audit flow
- extended release-surface audit coverage plus internal and external `production-ready` scaffold smoke so Forge now proves the final ship certificate exists, captures the latest compliance packet, and stays coherent with release output before the framework version can move
...
- added generated `pnpm ops:integrity` plus `scripts/ops-integrity.sh`, so scaffolded `production-ready` apps now turn the latest runbook, resilience handoff, rollback proof, gate verdict, attestation, and packaged release evidence into one final Markdown, JSON, and tarball integrity packet under `ops/integrity/`
- updated generated `production:check`, validate workflow, tagged release workflow, `ops:index`, and retention so scaffolded apps now inventory, upload, and keep `ops/integrity/` bounded as part of the standard production audit flow
- extended release-surface audit coverage plus internal and external `production-ready` scaffold smoke so Forge now proves the final operator integrity packet exists, contains the latest runbook packet, and stays coherent with release output before the framework version can move

## 0.1.142 (2026-04-07)

### Production Operator Runbooks

- added generated `pnpm ops:runbook` plus `scripts/ops-runbook.sh`, so scaffolded `production-ready` apps now turn the latest resilience handoff, continuity chain, escalation packet, rollback proof, gate verdict, attestation, and release evidence into one final operator Markdown, JSON, and tarball runbook under `ops/runbooks/`
- updated generated `production:check`, validate workflow, tagged release workflow, `ops:index`, and retention so scaffolded apps now inventory, upload, and keep `ops/runbooks/` bounded as part of the standard production audit flow
- extended release-surface audit coverage plus internal and external `production-ready` scaffold smoke so Forge now proves the latest resilience chain can be elevated into one portable operator runbook before the framework version can move

## 0.1.141 (2026-04-07)

### Production Resilience Handoffs

- added generated `pnpm ops:resilience` plus `scripts/ops-resilience.sh`, so scaffolded `production-ready` apps now turn the latest continuity handoff, escalation packet, recovery chain, gate verdict, attestation, and release evidence into one final disaster-recovery Markdown, JSON, and tarball handoff under `ops/resilience/`
- updated generated `production:check`, validate workflow, tagged release workflow, `ops:index`, and retention so scaffolded apps now inventory, upload, and keep `ops/resilience/` bounded as part of the standard production audit flow
- extended release-surface audit coverage plus internal and external `production-ready` scaffold smoke so Forge now proves the latest continuity chain can be elevated into one portable resilience handoff before the framework version can move
