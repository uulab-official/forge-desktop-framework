...
- generated apps now also get packaged-artifact verification scripts so local packaging can fail fast when release outputs are incomplete
...
- `scripts/setup-python.sh`, `scripts/build-worker.sh`, `scripts/build-app.sh`, `scripts/preflight-release.sh`, `scripts/check-publish-env.sh`, `scripts/verify-package-output.sh`
- package scripts for `pnpm release:check`, `pnpm publish:check:github`, `pnpm publish:check:s3`, `pnpm package:verify`, `pnpm package:verify:s3`, `pnpm publish:github`, `pnpm publish:s3`, `pnpm setup:python`, `pnpm build:worker`, and `pnpm package`
