...
- generated apps now also get `pnpm security:check`, `pnpm ops:check`, `pnpm ops:snapshot`, `pnpm ops:evidence`, and `pnpm production:check*` commands plus `docs/production-readiness.md` so production-grade validation can be rerun from one entry point
- generated validate and tagged release workflows now also upload `ops/snapshots/` and `ops/evidence/` so CI and release runs keep operator-facing evidence without extra setup
