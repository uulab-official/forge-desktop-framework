...
- tagged release metadata now also includes `one-point-zero-major-release-constitution.md/json` and `one-point-zero-major-release-covenant.md/json`, which turn that canon artifact plus the prepared `v1.0.0` checklist into the final immutable constitution and covenant surfaces for the first major ship
...
- maintainers can now also validate the final 1.0 major-release constitution and covenant surfaces with `pnpm release:major:constitution:test` and `pnpm release:major:covenant:test`
