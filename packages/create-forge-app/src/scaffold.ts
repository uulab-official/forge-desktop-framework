...
async function rewriteProjectReadme(
  ...
  try {
    ...
    const generated = [
      ...
      '- Run `pnpm ops:index` when you want one inventory view of the current `ops/snapshots/`, `ops/evidence/`, `ops/reports/`, `ops/bundles/`, `ops/doctors/`, `ops/handoffs/`, `ops/attestations/`, `ops/ready/`, `ops/gates/`, `ops/releasepacks/`, `ops/exports/`, `ops/restores/`, `ops/recoveries/`, `ops/rollbacks/`, and `ops/incidents/` directories',
...
function getValidateWorkflow(): string {
  return [
    ...
    '        run: pnpm build',
    '',
    '      - name: Operations incident packet',
    '        run: pnpm ops:incident -- --label validate --skip-retention',
    '',
...
function getReleaseWorkflow(): string {
  return [
    ...
    '',
    '      - name: Operations incident packet',
    '        run: pnpm ops:incident -- --label release-${{ matrix.platform }} --skip-retention --require-release-output',
    '',
    '      - name: Upload operations snapshots',
    '        if: always()',
    '        uses: actions/upload-artifact@v4',
...
function getReleasePlaybook(projectName: string, metadata: ScaffoldMetadata): string {
  ...
  return [
    ...
    'pnpm production:check',
    'pnpm package',
    'pnpm ops:incident -- --label release-smoke --require-release-output',
    '```',
    '',
...
function getProductionReadinessGuide(projectName: string, metadata: ScaffoldMetadata): string {
  ...
  return [
    ...
    'pnpm production:check',
    'pnpm package',
    'pnpm ops:incident -- --label github-readiness --require-release-output',
    '```',
    '',
    ...
    'pnpm package:s3',
    'pnpm ops:incident -- --label s3-readiness --require-release-output',
    '```',
    '',
    ...
    'pnpm package',
    'pnpm ops:incident -- --label full-audit --require-release-output',
    '```',
    '',
