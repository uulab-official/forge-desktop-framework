...
async function rewriteProjectReadme(
  ...
  try {
    ...
    const generated = [
      ...
      '- Run `pnpm ops:retention` to prune old `ops/snapshots/`, `ops/evidence/`, `ops/index/`, `ops/reports/`, `ops/bundles/`, `ops/doctors/`, `ops/handoffs/`, `ops/attestations/`, `ops/ready/`, `ops/gates/`, `ops/releasepacks/`, `ops/exports/`, `ops/restores/`, `ops/recoveries/`, `ops/rollbacks/`, `ops/incidents/`, `ops/escalations/`, `ops/continuity/`, `ops/resilience/`, `ops/runbooks/`, `ops/integrity/`, and `ops/compliance/` directories before or after repeated production checks',
...
function getValidateWorkflow(): string {
  return [
    ...
    '        run: pnpm build',
    '',
    '      - name: Operations compliance handoff',
    '        run: pnpm ops:compliance -- --label validate --skip-retention',
    '',
...
function getReleaseWorkflow(): string {
  return [
    ...
    '',
    '      - name: Operations compliance handoff',
    '        run: pnpm ops:compliance -- --label release-${{ matrix.platform }} --skip-retention --require-release-output',
    '',
    '      - name: Upload operations snapshots',
    '        if: always()',
    '        uses: actions/upload-artifact@v4',
...
function getReleasePlaybook(projectName: string, metadata: ScaffoldMetadata): string {
  ...
  return [
    ...
    'pnpm production:check',
    'pnpm package',
    'pnpm ops:compliance -- --label release-smoke --require-release-output',
    '```',
    '',
...
function getProductionReadinessGuide(projectName: string, metadata: ScaffoldMetadata): string {
  ...
  return [
    ...
    'pnpm production:check',
    'pnpm package',
    'pnpm ops:compliance -- --label github-readiness --require-release-output',
    '```',
    '',
    ...
    'pnpm package:s3',
    'pnpm ops:compliance -- --label s3-readiness --require-release-output',
    '```',
    '',
    ...
    'pnpm package',
    'pnpm ops:compliance -- --label full-audit --require-release-output',
    '```',
    '',
    ...
    '',
    '## Final Compliance Packet',
    '',
    '```bash',
    'pnpm ops:compliance -- --label release-compliance --require-release-output',
    '```',
    '',
    'Use `pnpm ops:compliance` when you want one final compliance packet under `ops/compliance/` that packages the latest integrity packet, runbook, resilience chain, gate verdict, attestation, release evidence, and operator docs into one final audit-ready handoff.',
    '',
    '## What Gets Checked',
    '',
