...
async function rewriteProjectReadme(
  ...
  try {
    ...
    const generated = [
      ...
      '- Run `pnpm ops:index` when you want one inventory view of the current `ops/snapshots/`, `ops/evidence/`, `ops/reports/`, `ops/bundles/`, `ops/doctors/`, `ops/handoffs/`, `ops/attestations/`, `ops/ready/`, `ops/gates/`, and `ops/releasepacks/` directories',
      ...
      '- Run `pnpm ops:retention` to prune old `ops/snapshots/`, `ops/evidence/`, `ops/index/`, `ops/reports/`, `ops/bundles/`, `ops/doctors/`, `ops/handoffs/`, `ops/attestations/`, `ops/ready/`, `ops/gates/`, and `ops/releasepacks/` directories before or after repeated production checks',
...
function getProductionReadinessScript(): string {
  return [
    ...
    'done',
    '',
    'echo "=== Operations release pack (${snapshot_label}) ==="',
    'releasepack_args=(-- --label "$snapshot_label" --skip-retention)',
...
function getOpsIndexScript(): string {
  return [
    ...
    '  counts: { snapshots: snapshots.length, evidence: evidence.length, reports: reports.length, bundles: bundles.length, doctors: doctors.length, handoffs: handoffs.length, attestations: attestations.length, ready: readys.length, gates: gates.length, releasePacks: releasePacks.length },',
    '  latest: {',
...
function getValidateWorkflow(): string {
  return [
    ...
    '        run: pnpm build',
    '',
    '      - name: Operations release pack',
    '        run: pnpm ops:releasepack -- --label validate --skip-retention',
    '',
...
function getReleaseWorkflow(): string {
  return [
    ...
    '',
    '      - name: Operations release pack',
    '        run: pnpm ops:releasepack -- --label release-${{ matrix.platform }} --skip-retention --require-release-output',
    '',
    '      - name: Upload operations snapshots',
    '        if: always()',
    '        uses: actions/upload-artifact@v4',
...
function getReleasePlaybook(projectName: string, metadata: ScaffoldMetadata): string {
  ...
  return [
    ...
    'pnpm production:check',
    'pnpm package',
    'pnpm ops:releasepack -- --label release-smoke --require-release-output',
    '```',
    '',
...
function getProductionReadinessGuide(projectName: string, metadata: ScaffoldMetadata): string {
  ...
  return [
    ...
    'pnpm production:check',
    'pnpm package',
    'pnpm ops:releasepack -- --label github-readiness --require-release-output',
    '```',
    '',
    ...
    'pnpm package:s3',
    'pnpm ops:releasepack -- --label s3-readiness --require-release-output',
    '```',
    '',
    ...
    'pnpm package',
    'pnpm ops:releasepack -- --label full-audit --require-release-output',
    '```',
    '',
    ...
    '',
    '```bash',
    'pnpm ops:releasepack -- --label release-pack --require-release-output',
    '```',
    '',
    'Use `pnpm ops:releasepack` when you want one final portable Markdown, JSON, and tarball release-evidence pack under `ops/releasepacks/` after the gate verdict and packaged release output are both in place.',
    '',
    '## What Gets Checked',
    '',
    ...
    '- Operations release pack under `ops/releasepacks/` so operators get one final portable tarball and evidence directory that includes the latest gate, handoff, attestation, ready, docs, env template, and packaged release output',
