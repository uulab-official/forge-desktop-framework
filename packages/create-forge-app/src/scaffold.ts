...
async function writeReleasePreset(
  ...
  await writeFile(join(targetDir, 'scripts', 'ops-stewardship.sh'), getOpsStewardshipScript(), 'utf-8');
...
function getOpsResilienceScript(): string {
  return [
    ...
    'NODE',
    '',
    'resilience_dir="$(node -e "const fs=require(\'fs\'); const path=require(\'path\'); const root=process.argv[1]; const dirs=fs.existsSync(root)?fs.readdirSync(root,{withFileTypes:true}).filter((entry)=>entry.isDirectory()).map((entry)=>path.join(root, entry.name)).sort():[]; if (dirs.length) process.stdout.write(dirs[dirs.length - 1]);" "$OUTPUT_ROOT")"',
...
function getOpsRunbookScript(): string {
  return [
    ...
    'NODE',
    '',
    'runbook_dir="$(node -e "const fs=require(\'fs\'); const path=require(\'path\'); const root=process.argv[1]; const dirs=fs.existsSync(root)?fs.readdirSync(root,{withFileTypes:true}).filter((entry)=>entry.isDirectory()).map((entry)=>path.join(root, entry.name)).sort():[]; if (dirs.length) process.stdout.write(dirs[dirs.length - 1]);" "$OUTPUT_ROOT")"',
...
function getOpsIntegrityScript(): string {
  return [
    ...
    'NODE',
    '',
    'integrity_dir="$(node -e "const fs=require(\'fs\'); const path=require(\'path\'); const root=process.argv[1]; const dirs=fs.existsSync(root)?fs.readdirSync(root,{withFileTypes:true}).filter((entry)=>entry.isDirectory()).map((entry)=>path.join(root, entry.name)).sort():[]; if (dirs.length) process.stdout.write(dirs[dirs.length - 1]);" "$OUTPUT_ROOT")"',
...
function getOpsStewardshipScript(): string {
  return [
    '#!/bin/bash',
    'set -euo pipefail',
    '',
...
function getValidateWorkflow(): string {
  return [
    ...
    '        run: pnpm build',
    '',
    '      - name: Operations stewardship handoff',
    '        run: pnpm ops:stewardship -- --label validate --skip-retention',
    '',
    ...
    '          if-no-files-found: error',
    '',
    '      - name: Upload operations stewardship',
    '        if: always()',
    '        uses: actions/upload-artifact@v4',
    '        with:',
    '          name: ops-stewardship-validate-${{ github.run_id }}',
    '          path: ops/stewardship',
    '          if-no-files-found: error',
    '',
  ].join('\n');
}
...
function getReleaseWorkflow(): string {
  return [
    ...
    '',
    '      - name: Operations stewardship handoff',
    '        run: pnpm ops:stewardship -- --label release-${{ matrix.platform }} --skip-retention --require-release-output',
    '',
    '      - name: Upload operations snapshots',
    '        if: always()',
    '        uses: actions/upload-artifact@v4',
    ...
    '        uses: actions/upload-artifact@v4',
    '        with:',
    '          name: ops-stewardship-${{ matrix.platform }}-${{ github.ref_name }}',
    '          path: ops/stewardship',
    '          if-no-files-found: error',
    '',
  ].join('\n');
