...
function getProductionReadinessScript(): string {
  return [
    ...
    '',
    'echo "=== Operations resilience handoff (${snapshot_label}) ==="',
    'resilience_args=(-- --label "$snapshot_label" --skip-retention)',
    'if [ "$REQUIRE_RELEASE_OUTPUT" -eq 1 ]; then',
...
function getValidateWorkflow(): string {
  return [
    ...
    '        run: pnpm build',
    '',
    '      - name: Operations resilience handoff',
    '        run: pnpm ops:resilience -- --label validate --skip-retention',
    '',
...
function getReleaseWorkflow(): string {
  return [
    ...
    '',
    '      - name: Operations resilience handoff',
    '        run: pnpm ops:resilience -- --label release-${{ matrix.platform }} --skip-retention --require-release-output',
    '',
    '      - name: Upload operations snapshots',
    '        if: always()',
    '        uses: actions/upload-artifact@v4',
...
function getReleasePlaybook(projectName: string, metadata: ScaffoldMetadata): string {
  ...
  return [
    ...
    'pnpm production:check',
    'pnpm package',
    'pnpm ops:resilience -- --label release-smoke --require-release-output',
    '```',
    '',
...
function getProductionReadinessGuide(projectName: string, metadata: ScaffoldMetadata): string {
  ...
  return [
    ...
    'pnpm production:check',
    'pnpm package',
    'pnpm ops:resilience -- --label github-readiness --require-release-output',
    '```',
    '',
    ...
    'pnpm package:s3',
    'pnpm ops:resilience -- --label s3-readiness --require-release-output',
    '```',
    '',
    ...
    'pnpm package',
    'pnpm ops:resilience -- --label full-audit --require-release-output',
    '```',
    '',
    ...
    '- Operations retention baseline so repeated production checks keep only the most recent snapshot, evidence, report, bundle, index, doctor, handoff, attestation, ready, gate, release pack, export, restore, recover, rollback, incident, escalation, continuity, and resilience directories by default',
