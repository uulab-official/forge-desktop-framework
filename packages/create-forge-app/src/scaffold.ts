...
async function rewriteProjectReadme(
  ...
  try {
    ...
    const generated = [
      ...
      '- Run `pnpm ops:retention` to prune old `ops/snapshots/`, `ops/evidence/`, `ops/index/`, `ops/reports/`, `ops/bundles/`, `ops/doctors/`, `ops/handoffs/`, `ops/attestations/`, `ops/ready/`, `ops/gates/`, `ops/releasepacks/`, `ops/exports/`, `ops/restores/`, `ops/recoveries/`, `ops/rollbacks/`, `ops/incidents/`, `ops/escalations/`, `ops/continuity/`, `ops/resilience/`, `ops/runbooks/`, `ops/integrity/`, `ops/compliance/`, `ops/certifications/`, `ops/assurances/`, `ops/governance/`, `ops/oversight/`, `ops/control/`, and `ops/authority/` directories before or after repeated production checks',
...
async function writeReleasePreset(
  ...
  await writeFile(join(targetDir, 'scripts', 'ops-rollback.sh'), getOpsRollbackScript(), 'utf-8');
  await writeFile(join(targetDir, 'scripts', 'ops-incident.sh'), getOpsIncidentScript(), 'utf-8');
  await writeFile(join(targetDir, 'scripts', 'ops-escalate.sh'), getOpsEscalateScript(), 'utf-8');
  await writeFile(join(targetDir, 'scripts', 'ops-continuity.sh'), getOpsContinuityScript(), 'utf-8');
  await writeFile(join(targetDir, 'scripts', 'ops-resilience.sh'), getOpsResilienceScript(), 'utf-8');
  await writeFile(join(targetDir, 'scripts', 'ops-runbook.sh'), getOpsRunbookScript(), 'utf-8');
  await writeFile(join(targetDir, 'scripts', 'ops-integrity.sh'), getOpsIntegrityScript(), 'utf-8');
  await writeFile(join(targetDir, 'scripts', 'ops-compliance.sh'), getOpsComplianceScript(), 'utf-8');
  await writeFile(join(targetDir, 'scripts', 'ops-certify.sh'), getOpsCertificationScript(), 'utf-8');
  await writeFile(join(targetDir, 'scripts', 'ops-assure.sh'), getOpsAssuranceScript(), 'utf-8');
  await writeFile(join(targetDir, 'scripts', 'ops-govern.sh'), getOpsGovernanceScript(), 'utf-8');
  await writeFile(join(targetDir, 'scripts', 'ops-oversight.sh'), getOpsOversightScript(), 'utf-8');
  await writeFile(join(targetDir, 'scripts', 'ops-control.sh'), getOpsControlScript(), 'utf-8');
  await writeFile(join(targetDir, 'scripts', 'ops-authority.sh'), getOpsAuthorityScript(), 'utf-8');
...
async function updatePackageJsonForRelease(
  ...
  pkg.scripts = {
    ...
    'ops:govern': 'bash scripts/ops-govern.sh',
...
function getOpsRollbackScript(): string {
  return [
    '#!/bin/bash',
    'set -euo pipefail',
    '',
...
function getOpsIncidentScript(): string {
  return [
    '#!/bin/bash',
    'set -euo pipefail',
    '',
    ...
    'NODE',
    '',
    'incident_dir="$(find "$OUTPUT_ROOT" -mindepth 1 -maxdepth 1 -type d | sort | tail -n 1)"',
...
function getOpsEscalateScript(): string {
  return [
    '#!/bin/bash',
    'set -euo pipefail',
    '',
    ...
    'NODE',
    '',
    'escalation_dir="$(find "$OUTPUT_ROOT" -mindepth 1 -maxdepth 1 -type d | sort | tail -n 1)"',
...
function getOpsContinuityScript(): string {
  return [
    '#!/bin/bash',
    'set -euo pipefail',
    '',
    ...
    'NODE',
    '',
    'continuity_dir="$(find "$OUTPUT_ROOT" -mindepth 1 -maxdepth 1 -type d | sort | tail -n 1)"',
...
function getOpsResilienceScript(): string {
  return [
    '#!/bin/bash',
    'set -euo pipefail',
    '',
    ...
    'cd "$ROOT_DIR"',
    '',
    'LABEL="${OPS_RESILIENCE_LABEL:-resilience}"',
    'OUTPUT_ROOT="${OPS_RESILIENCE_DIR:-ops/resilience}"',
    'KEEP="${OPS_RETENTION_KEEP:-10}"',
    'REQUIRE_RELEASE_OUTPUT=0',
    'SKIP_RETENTION=0',
    '',
    ...
    '      ;;',
    '    *)',
    '      echo "Unsupported ops resilience argument: $1"',
    '      echo "Use --label <name>, --output-dir <path>, --keep <count>, --require-release-output, and --skip-retention."',
    '      exit 1',
    '      ;;',
    '  esac',
    ...
    'fi',
    '',
    'continuity_args=(-- --label "$LABEL" --skip-retention)',
    'if [ "$REQUIRE_RELEASE_OUTPUT" -eq 1 ]; then',
    '  continuity_args+=(--require-release-output)',
    'fi',
    'pnpm ops:continuity "${continuity_args[@]}"',
    ...
    'addCheck(\'gate-go\', gate?.payload?.ok === true, gate ? `gate verdict=${gate.payload?.ok ? \'go\' : \'no-go\'}` : \'gate missing\', checks);',
    'addCheck(\'attestation-pass\', attestation?.payload?.ok === true, attestation ? `attestation verdict=${attestation.payload?.ok ? \'pass\' : \'fail\'}` : \'attestation missing\', checks);',
    'if (index?.payload?.latest?.continuity?.relativeDir && continuity) {',
    '  addCheck(\'index-latest-continuity-aligned\', index.payload.latest.continuity.relativeDir === continuity.relativeDir, `index continuity=${index.payload.latest.continuity.relativeDir}`, checks);',
    '}',
    '',
    'if (requireReleaseOutput) {',
    '  addCheck(\'release-manifests\', manifestFiles.length > 0, manifestFiles.length > 0 ? manifestFiles.join(\', \') : \'no release manifests\', checks);',
    ...
    'const safeLabel = String(label).toLowerCase().replace(/[^a-z0-9._-]+/g, \'-\').replace(/^-+|-+$/g, \'\') || \'manual\';',
    'const timestamp = capturedAt.replace(/[-:]/g, \'\').replace(/\\.\\d+Z$/, \'Z\');',
    'const resilienceDir = path.join(outputRoot, `${timestamp}-${safeLabel}`);',
    'const packetDir = path.join(resilienceDir, \'packet\');',
    'fs.mkdirSync(packetDir, { recursive: true });',
    '',
    'const copySurface = (surface, key, names) => {',
    '  if (!surface) return [];',
    ...
    '};',
    '',
    'const continuityPacketDir = continuity ? path.join(continuity.absoluteDir, \'packet\') : null;',
    'if (copyDirIfExists(continuityPacketDir, path.join(packetDir, \'continuity\', \'packet\'))) {',
    '  copied.continuityPacket.push(path.join(\'packet\', \'continuity\', \'packet\'));',
    '}',
    'const escalationPacketDir = escalate ? path.join(escalate.absoluteDir, \'packet\') : null;',
    'if (copyDirIfExists(escalationPacketDir, path.join(packetDir, \'escalation\', \'packet\'))) {',
    '  copied.escalationPacket.push(path.join(\'packet\', \'escalation\', \'packet\'));',
    ...
    '}',
    '',
    'const rerunCommand = `pnpm ops:resilience -- --label ${safeLabel}${requireReleaseOutput ? \' --require-release-output\' : \'\'}${skipRetention ? \' --skip-retention\' : \'\'} `.trim();',
    'const ok = checks.every((check) => check.ok);',
    'const summary = {',
    '  capturedAt,',
    '  label,',
    '  ok,',
    '  package: { name: pkg.name, version: pkg.version },',
    '  resilienceDir: path.relative(rootDir, resilienceDir),',
    '  archivePath: path.join(path.relative(rootDir, resilienceDir), \'ops-resilience.tgz\'),',
    '  packetDir: path.join(path.relative(rootDir, resilienceDir), \'packet\'),',
    '  rerunCommand,',
    '  requireReleaseOutput,',
    '  retention: { skipped: skipRetention, keep: Number(keep) },',
    '  latest: {',
    ...
    '};',
    '',
    'fs.writeFileSync(path.join(resilienceDir, \'ops-resilience.json\'), JSON.stringify(summary, null, 2) + \'\\n\');',
    'const lines = [',
    ...
    '  `- Package: ${summary.package.name}@${summary.package.version}`,',
    '  `- Verdict: ${summary.ok ? \'pass\' : \'fail\'}`,',
    '  `- Resilience Dir: ${summary.resilienceDir}`,',
    '  `- Archive Path: ${summary.archivePath}`,',
    '  `- Packet Dir: ${summary.packetDir}`,',
    '  `- Rerun Command: ${summary.rerunCommand}`,',
    '  `- Release output required: ${summary.requireReleaseOutput ? \'yes\' : \'no\'}`,',
    ...
    '  `- Continuity packet trees: ${summary.copied.continuityPacket.length}`,',
    '  `- Escalation files: ${summary.copied.escalation.length}`,',
    '  `- Escalation packet trees: ${summary.copied.escalationPacket.length}`,',
    '  `- Incident files: ${summary.copied.incident.length}`,',
    '  `- Rollback files: ${summary.copied.rollback.length}`,',
    ...
    '  \'## Operator Next Steps\',',
    '  \'\',',
    '  \'- Attach `ops-resilience.tgz` when the latest continuity, escalation, rollback, recovery, gate, attestation, and release evidence must travel together as one disaster-recovery handoff.\',',
    '  \'- Use `ops-resilience.json` as the machine-readable resilience record for incident command, business-continuity review, or release rollback governance.\',',
    '  \'- Re-run `pnpm ops:resilience -- --label <name> --require-release-output` after any continuity, escalation, rollback, export, or packaged release change.\',',
    '  \'\',',
    '];',
    'fs.writeFileSync(path.join(resilienceDir, \'ops-resilience.md\'), lines.join(\'\\n\') + \'\\n\');',
    'console.log(`Operations resilience written to: ${resilienceDir}`);',
    'if (!ok) process.exit(1);',
    'NODE',
    '',
    'resilience_dir="$(find "$OUTPUT_ROOT" -mindepth 1 -maxdepth 1 -type d | sort | tail -n 1)"',
    'if [ -z "$resilience_dir" ]; then',
    '  echo "Failed to locate generated ops resilience directory in $OUTPUT_ROOT"',
    '  exit 1',
    'fi',
    'tar -czf "$resilience_dir/ops-resilience.tgz" -C "$resilience_dir" packet',
    'echo "Operations resilience archive written to: $resilience_dir/ops-resilience.tgz"',
    'pnpm ops:index -- --label "$LABEL"',
    '',
  ].join('\n');
}

function getOpsRunbookScript(): string {
  return [
    '#!/bin/bash',
    'set -euo pipefail',
    '',
    ...
    'cd "$ROOT_DIR"',
    '',
    'LABEL="${OPS_RUNBOOK_LABEL:-runbook}"',
    'OUTPUT_ROOT="${OPS_RUNBOOK_DIR:-ops/runbooks}"',
    'KEEP="${OPS_RETENTION_KEEP:-10}"',
    'REQUIRE_RELEASE_OUTPUT=0',
    'SKIP_RETENTION=0',
    '',
    ...
    '      ;;',
    '    *)',
    '      echo "Unsupported ops runbook argument: $1"',
    '      echo "Use --label <name>, --output-dir <path>, --keep <count>, --require-release-output, and --skip-retention."',
    '      exit 1',
    '      ;;',
    '  esac',
    ...
    'fi',
    '',
    'resilience_args=(-- --label "$LABEL" --skip-retention)',
    'if [ "$REQUIRE_RELEASE_OUTPUT" -eq 1 ]; then',
    '  resilience_args+=(--require-release-output)',
    'fi',
    'pnpm ops:resilience "${resilience_args[@]}"',
    '',
    'mkdir -p "$OUTPUT_ROOT"',
    '',
    'node - "$ROOT_DIR" "$OUTPUT_ROOT" "$LABEL" "$REQUIRE_RELEASE_OUTPUT" "$SKIP_RETENTION" "$KEEP" <<\'NODE\'',
    ...
    'addCheck(\'gate-go\', gate?.payload?.ok === true, gate ? `gate verdict=${gate.payload?.ok ? \'go\' : \'no-go\'}` : \'gate missing\', checks);',
    'addCheck(\'attestation-pass\', attestation?.payload?.ok === true, attestation ? `attestation verdict=${attestation.payload?.ok ? \'pass\' : \'fail\'}` : \'attestation missing\', checks);',
    'if (index?.payload?.latest?.resilience?.relativeDir && resilience) {',
    '  addCheck(\'index-latest-resilience-aligned\', index.payload.latest.resilience.relativeDir === resilience.relativeDir, `index resilience=${index.payload.latest.resilience.relativeDir}`, checks);',
    '}',
    '',
    'if (requireReleaseOutput) {',
    '  addCheck(\'release-manifests\', manifestFiles.length > 0, manifestFiles.length > 0 ? manifestFiles.join(\', \') : \'no release manifests\', checks);',
    ...
    'const safeLabel = String(label).toLowerCase().replace(/[^a-z0-9._-]+/g, \'-\').replace(/^-+|-+$/g, \'\') || \'manual\';',
    'const timestamp = capturedAt.replace(/[-:]/g, \'\').replace(/\\.\\d+Z$/, \'Z\');',
    'const runbookDir = path.join(outputRoot, `${timestamp}-${safeLabel}`);',
    'const packetDir = path.join(runbookDir, \'packet\');',
    'fs.mkdirSync(packetDir, { recursive: true });',
    '',
    'const copySurface = (surface, key, names) => {',
    '  if (!surface) return [];',
    ...
    '};',
    '',
    'const resiliencePacketDir = resilience ? path.join(resilience.absoluteDir, \'packet\') : null;',
    'if (copyDirIfExists(resiliencePacketDir, path.join(packetDir, \'resilience\', \'packet\'))) {',
    '  copied.resiliencePacket.push(path.join(\'packet\', \'resilience\', \'packet\'));',
    '}',
    'const continuityPacketDir = continuity ? path.join(continuity.absoluteDir, \'packet\') : null;',
    'if (copyDirIfExists(continuityPacketDir, path.join(packetDir, \'continuity\', \'packet\'))) {',
    '  copied.continuityPacket.push(path.join(\'packet\', \'continuity\', \'packet\'));',
    ...
    '}',
    '',
    'const rerunCommand = `pnpm ops:runbook -- --label ${safeLabel}${requireReleaseOutput ? \' --require-release-output\' : \'\'}${skipRetention ? \' --skip-retention\' : \'\'} `.trim();',
    'const ok = checks.every((check) => check.ok);',
    'const summary = {',
    '  capturedAt,',
    '  label,',
    '  ok,',
    '  package: { name: pkg.name, version: pkg.version },',
    '  runbookDir: path.relative(rootDir, runbookDir),',
    '  archivePath: path.join(path.relative(rootDir, runbookDir), \'ops-runbook.tgz\'),',
    '  packetDir: path.join(path.relative(rootDir, runbookDir), \'packet\'),',
    '  rerunCommand,',
    '  requireReleaseOutput,',
    '  retention: { skipped: skipRetention, keep: Number(keep) },',
    '  recommendedCommands: [',
    '    rerunCommand,',
    '    `pnpm ops:resilience -- --label ${safeLabel}${requireReleaseOutput ? \' --require-release-output\' : \'\'}${skipRetention ? \' --skip-retention\' : \'\'} `.trim(),',
    '    `pnpm ops:rollback -- --label ${safeLabel}${requireReleaseOutput ? \' --require-release-output\' : \'\'}${skipRetention ? \' --skip-retention\' : \'\'} `.trim(),',
    '  ],',
    '  latest: {',
    ...
    '};',
    '',
    'fs.writeFileSync(path.join(runbookDir, \'ops-runbook.json\'), JSON.stringify(summary, null, 2) + \'\\n\');',
    'const lines = [',
    '  \'# Operations Runbook\',',
    '  \'\',',
    '  `- Captured At (UTC): ${summary.capturedAt}`,',
    ...
    '  `- Package: ${summary.package.name}@${summary.package.version}`,',
    '  `- Verdict: ${summary.ok ? \'pass\' : \'fail\'}`,',
    '  `- Runbook Dir: ${summary.runbookDir}`,',
    '  `- Archive Path: ${summary.archivePath}`,',
    '  `- Packet Dir: ${summary.packetDir}`,',
    '  `- Rerun Command: ${summary.rerunCommand}`,',
    '  `- Release output required: ${summary.requireReleaseOutput ? \'yes\' : \'no\'}`,',
    ...
    '  `- Resilience packet trees: ${summary.copied.resiliencePacket.length}`,',
    '  `- Continuity files: ${summary.copied.continuity.length}`,',
    '  `- Continuity packet trees: ${summary.copied.continuityPacket.length}`,',
    '  `- Escalation files: ${summary.copied.escalation.length}`,',
    '  `- Incident files: ${summary.copied.incident.length}`,',
    ...
    '  \'## Operator Next Steps\',',
    '  \'\',',
    '  \'- Attach `ops-runbook.tgz` when release operators need one portable recovery execution guide that already contains the latest resilience, continuity, escalation, rollback, gate, attestation, and release evidence chain.\',',
    '  \'- Use `ops-runbook.json` as the machine-readable final handoff record for operator review, release command execution, or recovery rehearsal sign-off.\',',
    '  \'- Re-run `pnpm ops:runbook -- --label <name> --require-release-output` after any resilience, continuity, rollback, export, or packaged release change.\',',
    '  \'\',',
    '];',
    'fs.writeFileSync(path.join(runbookDir, \'ops-runbook.md\'), lines.join(\'\\n\') + \'\\n\');',
    'console.log(`Operations runbook written to: ${runbookDir}`);',
    'if (!ok) process.exit(1);',
    'NODE',
    '',
    'runbook_dir="$(find "$OUTPUT_ROOT" -mindepth 1 -maxdepth 1 -type d | sort | tail -n 1)"',
    'if [ -z "$runbook_dir" ]; then',
    '  echo "Failed to locate generated ops runbook directory in $OUTPUT_ROOT"',
    '  exit 1',
    'fi',
    'tar -czf "$runbook_dir/ops-runbook.tgz" -C "$runbook_dir" packet',
    'echo "Operations runbook archive written to: $runbook_dir/ops-runbook.tgz"',
    'pnpm ops:index -- --label "$LABEL"',
    '',
  ].join('\n');
}

function getOpsIntegrityScript(): string {
  return [
    '#!/bin/bash',
    'set -euo pipefail',
    '',
    ...
    '',
    'LABEL="manual"',
    'OUTPUT_ROOT="${OPS_INTEGRITY_DIR:-ops/integrity}"',
    'KEEP="${OPS_RETENTION_KEEP:-10}"',
    'REQUIRE_RELEASE_OUTPUT=0',
    'SKIP_RETENTION=0',
    '',
    ...
    '      ;;',
    '    *)',
    '      echo "Unsupported ops integrity argument: $1"',
    ...
    'fi',
    '',
    'runbook_args=(-- --label "$LABEL" --skip-retention)',
    'if [ "$REQUIRE_RELEASE_OUTPUT" -eq 1 ]; then',
    '  runbook_args+=(--require-release-output)',
    'fi',
    'pnpm ops:runbook "${runbook_args[@]}"',
    ...
    'addCheck(\'runbook-archive\', runbook ? fs.existsSync(path.join(runbook.absoluteDir, \'ops-runbook.tgz\')) : false, runbook ? \'ops-runbook.tgz present\' : \'runbook missing\', checks);',
    'addCheck(\'runbook-packet-resilience\', runbook ? fs.existsSync(path.join(runbook.absoluteDir, \'packet\', \'resilience\', \'ops-resilience.json\')) : false, runbook ? \'runbook packet contains resilience proof\' : \'runbook missing\', checks);',
    'addCheck(\'resilience-pass\', resilience?.payload?.ok === true, resilience ? `resilience verdict=${resilience.payload?.ok ? \'pass\' : \'fail\'}` : \'resilience missing\', checks);',
    'addCheck(\'continuity-pass\', continuity?.payload?.ok === true, continuity ? `continuity verdict=${continuity.payload?.ok ? \'pass\' : \'fail\'}` : \'continuity missing\', checks);',
    ...
    'addCheck(\'gate-go\', gate?.payload?.ok === true, gate ? `gate verdict=${gate.payload?.ok ? \'go\' : \'no-go\'}` : \'gate missing\', checks);',
    'addCheck(\'attestation-pass\', attestation?.payload?.ok === true, attestation ? `attestation verdict=${attestation.payload?.ok ? \'pass\' : \'fail\'}` : \'attestation missing\', checks);',
    'if (index?.payload?.latest?.runbook?.relativeDir && runbook) {',
    '  addCheck(\'index-latest-runbook-aligned\', index.payload.latest.runbook.relativeDir === runbook.relativeDir, `index runbook=${index.payload.latest.runbook.relativeDir}`, checks);',
    '}',
    'if (index?.payload?.latest?.resilience?.relativeDir && resilience) {',
    '  addCheck(\'index-latest-resilience-aligned\', index.payload.latest.resilience.relativeDir === resilience.relativeDir, `index resilience=${index.payload.latest.resilience.relativeDir}`, checks);',
    '}',
    ...
    'const safeLabel = String(label).toLowerCase().replace(/[^a-z0-9._-]+/g, \'-\').replace(/^-+|-+$/g, \'\') || \'manual\';',
    'const timestamp = capturedAt.replace(/[-:]/g, \'\').replace(/\\.\\d+Z$/, \'Z\');',
    'const integrityDir = path.join(outputRoot, `${timestamp}-${safeLabel}`);',
    'const packetDir = path.join(integrityDir, \'packet\');',
    'fs.mkdirSync(packetDir, { recursive: true });',
    '',
    'const copySurface = (surface, key, names) => {',
    '  if (!surface) return [];',
    ...
    '};',
    '',
    'const runbookPacketDir = runbook ? path.join(runbook.absoluteDir, \'packet\') : null;',
    'if (copyDirIfExists(runbookPacketDir, path.join(packetDir, \'runbook\', \'packet\'))) {',
    '  copied.runbookPacket.push(path.join(\'packet\', \'runbook\', \'packet\'));',
    '}',
    'for (const relativeDoc of [path.join(\'docs\', \'release-playbook.md\'), path.join(\'docs\', \'production-readiness.md\')]) {',
    '  const source = path.join(rootDir, relativeDoc);',
    '  const destination = path.join(packetDir, \'docs\', path.basename(relativeDoc));',
    ...
    '}',
    '',
    'const rerunCommand = `pnpm ops:integrity -- --label ${safeLabel}${requireReleaseOutput ? \' --require-release-output\' : \'\'}${skipRetention ? \' --skip-retention\' : \'\'} `.trim();',
    'const ok = checks.every((check) => check.ok);',
    'const summary = {',
    '  capturedAt,',
    '  label,',
    '  ok,',
    '  package: { name: pkg.name, version: pkg.version },',
    '  integrityDir: path.relative(rootDir, integrityDir),',
    '  archivePath: path.join(path.relative(rootDir, integrityDir), \'ops-integrity.tgz\'),',
    '  packetDir: path.join(path.relative(rootDir, integrityDir), \'packet\'),',
    '  rerunCommand,',
    '  requireReleaseOutput,',
    '  retention: { skipped: skipRetention, keep: Number(keep) },',
    '  recommendedCommands: [',
    ...
    '};',
    '',
    'fs.writeFileSync(path.join(integrityDir, \'ops-integrity.json\'), JSON.stringify(summary, null, 2) + \'\\n\');',
    'const lines = [',
    ...
    '  `- Package: ${summary.package.name}@${summary.package.version}`,',
    '  `- Verdict: ${summary.ok ? \'pass\' : \'fail\'}`,',
    '  `- Integrity Dir: ${summary.integrityDir}`,',
    '  `- Archive Path: ${summary.archivePath}`,',
    '  `- Packet Dir: ${summary.packetDir}`,',
    '  `- Rerun Command: ${summary.rerunCommand}`,',
    '  `- Release output required: ${summary.requireReleaseOutput ? \'yes\' : \'no\'}`,',
    ...
    '  `- Runbook packet trees: ${summary.copied.runbookPacket.length}`,',
    '  `- Resilience files: ${summary.copied.resilience.length}`,',
    '  `- Continuity files: ${summary.copied.continuity.length}`,',
    '  `- Escalation files: ${summary.copied.escalation.length}`,',
    '  `- Incident files: ${summary.copied.incident.length}`,',
    ...
    '  \'## Operator Next Steps\',',
    '  \'\',',
    '  \'- Attach `ops-integrity.tgz` when release operators need one portable integrity packet that proves the latest runbook and recovery evidence chain still agree after packaging.\',',
    '  \'- Use `ops-integrity.json` as the machine-readable final integrity verdict before publishing, signing off, or escalating a release to another team.\',',
    '  \'- Re-run `pnpm ops:integrity -- --label <name> --require-release-output` after any runbook, resilience, rollback, export, or packaged release change.\',',
    '  \'\',',
    '];',
    'fs.writeFileSync(path.join(integrityDir, \'ops-integrity.md\'), lines.join(\'\\n\') + \'\\n\');',
    'console.log(`Operations integrity written to: ${integrityDir}`);',
    'if (!ok) process.exit(1);',
    'NODE',
    '',
    'integrity_dir="$(find "$OUTPUT_ROOT" -mindepth 1 -maxdepth 1 -type d | sort | tail -n 1)"',
    'if [ -z "$integrity_dir" ]; then',
    '  echo "Failed to locate generated ops integrity directory in $OUTPUT_ROOT"',
    '  exit 1',
    'fi',
    'tar -czf "$integrity_dir/ops-integrity.tgz" -C "$integrity_dir" packet',
    'echo "Operations integrity archive written to: $integrity_dir/ops-integrity.tgz"',
    'pnpm ops:index -- --label "$LABEL"',
    '',
  ].join('\n');
}

function getOpsComplianceScript(): string {
  return [
    '#!/bin/bash',
    'set -euo pipefail',
    '',
    ...
    'NODE',
    '',
    'compliance_dir="$(find "$OUTPUT_ROOT" -mindepth 1 -maxdepth 1 -type d | sort | tail -n 1)"',
...
function getOpsCertificationScript(): string {
  return [
    '#!/bin/bash',
    'set -euo pipefail',
    '',
    ...
    'NODE',
    '',
    'certification_dir="$(find "$OUTPUT_ROOT" -mindepth 1 -maxdepth 1 -type d | sort | tail -n 1)"',
...
function getOpsAssuranceScript(): string {
  return [
    '#!/bin/bash',
    'set -euo pipefail',
    '',
    ...
    'NODE',
    '',
    'assurance_dir="$(find "$OUTPUT_ROOT" -mindepth 1 -maxdepth 1 -type d | sort | tail -n 1)"',
...
function getOpsGovernanceScript(): string {
  return [
    '#!/bin/bash',
    'set -euo pipefail',
    '',
    ...
    'NODE',
    '',
    'governance_dir="$(find "$OUTPUT_ROOT" -mindepth 1 -maxdepth 1 -type d | sort | tail -n 1)"',
...
function getOpsOversightScript(): string {
  return [
    '#!/bin/bash',
    'set -euo pipefail',
    '',
    ...
    'NODE',
    '',
    'oversight_dir="$(find "$OUTPUT_ROOT" -mindepth 1 -maxdepth 1 -type d | sort | tail -n 1)"',
...
function getOpsControlScript(): string {
  return [
    '#!/bin/bash',
    'set -euo pipefail',
    '',
    ...
    'NODE',
    '',
    'control_dir="$(find "$OUTPUT_ROOT" -mindepth 1 -maxdepth 1 -type d | sort | tail -n 1)"',
...
function getOpsAuthorityScript(): string {
  return [
    '#!/bin/bash',
    'set -euo pipefail',
    '',
    ...
    '  addCheck(\'index-recency-vs-control\', !index || Date.parse(index.modifiedAt) >= Date.parse(control.modifiedAt), !index ? \'index missing\' : Date.parse(index.modifiedAt) >= Date.parse(control.modifiedAt) ? \'index is newer than control\' : \'index is older than control\', checks);',
    '}',
    '',
    'const manifestFiles = fs.existsSync(releaseDir)',
    '  ? fs.readdirSync(releaseDir).filter((file) => /^latest.*\\.ya?ml$/i.test(file))',
    ...
    'const oversightPacketDir = oversight ? path.join(oversight.absoluteDir, \'packet\') : null;',
    'if (copyDirWithoutArchives(oversightPacketDir, path.join(packetDir, \'oversight\', \'packet\'))) copied.oversightPacket.push(path.join(\'packet\', \'oversight\', \'packet\'));',
    'for (const relativeDoc of [path.join(\'docs\', \'release-playbook.md\'), path.join(\'docs\', \'production-readiness.md\')]) {',
    '  const source = path.join(rootDir, relativeDoc);',
    '  const destination = path.join(packetDir, \'docs\', path.basename(relativeDoc));',
    '  if (copyIfExists(source, destination)) copied.docs.push(relativeDoc);',
    ...
    'NODE',
    '',
    'authority_dir="$(find "$OUTPUT_ROOT" -mindepth 1 -maxdepth 1 -type d | sort | tail -n 1)"',
...
function getValidateWorkflow(): string {
  return [
    ...
    '        run: pnpm build',
    '',
    '      - name: Operations authority handoff',
    '        run: pnpm ops:authority -- --label validate --skip-retention',
    '',
    ...
    '          if-no-files-found: error',
    '',
    '      - name: Upload operations rollbacks',
    '        if: always()',
    '        uses: actions/upload-artifact@v4',
    '        with:',
    '          name: ops-rollbacks-validate-${{ github.run_id }}',
    '          path: ops/rollbacks',
    '          if-no-files-found: error',
    '',
    '      - name: Upload operations incidents',
    '        if: always()',
    '        uses: actions/upload-artifact@v4',
    '        with:',
    '          name: ops-incidents-validate-${{ github.run_id }}',
    '          path: ops/incidents',
    '          if-no-files-found: error',
    '',
    '      - name: Upload operations escalations',
    '        if: always()',
    '        uses: actions/upload-artifact@v4',
    '        with:',
    '          name: ops-escalations-validate-${{ github.run_id }}',
    '          path: ops/escalations',
    '          if-no-files-found: error',
    '',
    '      - name: Upload operations continuity',
    '        if: always()',
    '        uses: actions/upload-artifact@v4',
    '        with:',
    '          name: ops-continuity-validate-${{ github.run_id }}',
    '          path: ops/continuity',
    '          if-no-files-found: error',
    '',
    '      - name: Upload operations resilience',
    '        if: always()',
    '        uses: actions/upload-artifact@v4',
    '        with:',
    '          name: ops-resilience-validate-${{ github.run_id }}',
    '          path: ops/resilience',
    '          if-no-files-found: error',
    '',
    '      - name: Upload operations runbooks',
    '        if: always()',
    '        uses: actions/upload-artifact@v4',
    '        with:',
    '          name: ops-runbooks-validate-${{ github.run_id }}',
    '          path: ops/runbooks',
    '          if-no-files-found: error',
    '',
    '      - name: Upload operations integrity',
    '        if: always()',
    '        uses: actions/upload-artifact@v4',
    '        with:',
    '          name: ops-integrity-validate-${{ github.run_id }}',
    '          path: ops/integrity',
    '          if-no-files-found: error',
    '',
    '      - name: Upload operations compliance',
    '        if: always()',
    '        uses: actions/upload-artifact@v4',
    '        with:',
    '          name: ops-compliance-validate-${{ github.run_id }}',
    '          path: ops/compliance',
    '          if-no-files-found: error',
    '',
    '      - name: Upload operations certifications',
    '        if: always()',
    '        uses: actions/upload-artifact@v4',
    '        with:',
    '          name: ops-certifications-validate-${{ github.run_id }}',
    '          path: ops/certifications',
    '          if-no-files-found: error',
    '',
    '      - name: Upload operations assurances',
    '        if: always()',
    '        uses: actions/upload-artifact@v4',
    '        with:',
    '          name: ops-assurances-validate-${{ github.run_id }}',
    '          path: ops/assurances',
    '          if-no-files-found: error',
    '',
    '      - name: Upload operations governance',
    '        if: always()',
    '        uses: actions/upload-artifact@v4',
    '        with:',
    '          name: ops-governance-validate-${{ github.run_id }}',
    '          path: ops/governance',
    '          if-no-files-found: error',
    '',
    '      - name: Upload operations oversight',
    '        if: always()',
    '        uses: actions/upload-artifact@v4',
    '        with:',
    '          name: ops-oversight-validate-${{ github.run_id }}',
    '          path: ops/oversight',
    '          if-no-files-found: error',
    '',
    '      - name: Upload operations control',
    '        if: always()',
    '        uses: actions/upload-artifact@v4',
    '        with:',
    '          name: ops-control-validate-${{ github.run_id }}',
    '          path: ops/control',
    '          if-no-files-found: error',
    '',
    '      - name: Upload operations authority',
    '        if: always()',
    '        uses: actions/upload-artifact@v4',
    '        with:',
    '          name: ops-authority-validate-${{ github.run_id }}',
    '          path: ops/authority',
    '          if-no-files-found: error',
    '',
  ].join('\n');
...
function getReleaseWorkflow(): string {
  return [
    ...
    '',
    '      - name: Operations authority handoff',
    '        run: pnpm ops:authority -- --label release-${{ matrix.platform }} --skip-retention --require-release-output',
    '',
    '      - name: Upload operations snapshots',
    '        if: always()',
    '        uses: actions/upload-artifact@v4',
    ...
    '        uses: actions/upload-artifact@v4',
    '        with:',
    '          name: ops-rollbacks-${{ matrix.platform }}-${{ github.ref_name }}',
    '          path: ops/rollbacks',
    '          if-no-files-found: error',
    '',
    '      - name: Upload operations incidents',
    ...
    '        uses: actions/upload-artifact@v4',
    '        with:',
    '          name: ops-incidents-${{ matrix.platform }}-${{ github.ref_name }}',
    '          path: ops/incidents',
    '          if-no-files-found: error',
    '',
    '      - name: Upload operations escalations',
    ...
    '        uses: actions/upload-artifact@v4',
    '        with:',
    '          name: ops-escalations-${{ matrix.platform }}-${{ github.ref_name }}',
    '          path: ops/escalations',
    '          if-no-files-found: error',
    '',
    '      - name: Upload operations continuity',
    ...
    '        uses: actions/upload-artifact@v4',
    '        with:',
    '          name: ops-continuity-${{ matrix.platform }}-${{ github.ref_name }}',
    '          path: ops/continuity',
    '          if-no-files-found: error',
    '',
    '      - name: Upload operations resilience',
    ...
    '        uses: actions/upload-artifact@v4',
    '        with:',
    '          name: ops-resilience-${{ matrix.platform }}-${{ github.ref_name }}',
    '          path: ops/resilience',
    '          if-no-files-found: error',
    '',
    '      - name: Upload operations runbooks',
    ...
    '        uses: actions/upload-artifact@v4',
    '        with:',
    '          name: ops-runbooks-${{ matrix.platform }}-${{ github.ref_name }}',
    '          path: ops/runbooks',
    '          if-no-files-found: error',
    '',
    '      - name: Upload operations integrity',
    ...
    '        uses: actions/upload-artifact@v4',
    '        with:',
    '          name: ops-integrity-${{ matrix.platform }}-${{ github.ref_name }}',
    '          path: ops/integrity',
    '          if-no-files-found: error',
    '',
    '      - name: Upload operations compliance',
    ...
    '        uses: actions/upload-artifact@v4',
    '        with:',
    '          name: ops-compliance-${{ matrix.platform }}-${{ github.ref_name }}',
    '          path: ops/compliance',
    '          if-no-files-found: error',
    '',
    '      - name: Upload operations certifications',
    ...
    '        uses: actions/upload-artifact@v4',
    '        with:',
    '          name: ops-certifications-${{ matrix.platform }}-${{ github.ref_name }}',
    '          path: ops/certifications',
    '          if-no-files-found: error',
    '',
    '      - name: Upload operations assurances',
    ...
    '        uses: actions/upload-artifact@v4',
    '        with:',
    '          name: ops-assurances-${{ matrix.platform }}-${{ github.ref_name }}',
    '          path: ops/assurances',
    '          if-no-files-found: error',
    '',
    '      - name: Upload operations governance',
    ...
    '        uses: actions/upload-artifact@v4',
    '        with:',
    '          name: ops-governance-${{ matrix.platform }}-${{ github.ref_name }}',
    '          path: ops/governance',
    '          if-no-files-found: error',
    '',
    '      - name: Upload operations oversight',
    ...
    '        uses: actions/upload-artifact@v4',
    '        with:',
    '          name: ops-oversight-${{ matrix.platform }}-${{ github.ref_name }}',
    '          path: ops/oversight',
    '          if-no-files-found: error',
    '',
    '      - name: Upload operations control',
    ...
    '        uses: actions/upload-artifact@v4',
    '        with:',
    '          name: ops-control-${{ matrix.platform }}-${{ github.ref_name }}',
    '          path: ops/control',
    '          if-no-files-found: error',
    '',
    '      - name: Upload operations authority',
    ...
    '        uses: actions/upload-artifact@v4',
    '        with:',
    '          name: ops-authority-${{ matrix.platform }}-${{ github.ref_name }}',
    '          path: ops/authority',
    '          if-no-files-found: error',
    '',
  ].join('\n');
...
function getReleasePlaybook(projectName: string, metadata: ScaffoldMetadata): string {
  ...
  return [
    ...
    'pnpm production:check',
    'pnpm package',
    'pnpm ops:authority -- --label release-smoke --require-release-output',
    '```',
    '',
    ...
    '```',
    '',
    'Use `pnpm ops:oversight` when you want the final production oversight packet under `ops/oversight/`, `pnpm ops:control` when you want one last operator-facing control packet under `ops/control/`, and `pnpm ops:authority` when you want the final production authority packet under `ops/authority/` before close-out.',
    '',
...
function getProductionReadinessGuide(projectName: string, metadata: ScaffoldMetadata): string {
  ...
  return [
    ...
    'pnpm production:check',
    'pnpm package',
    'pnpm ops:authority -- --label github-readiness --require-release-output',
    '```',
    '',
    ...
    'pnpm package:s3',
    'pnpm ops:authority -- --label s3-readiness --require-release-output',
    '```',
    '',
    ...
    'pnpm package',
    'pnpm ops:authority -- --label full-audit --require-release-output',
    '```',
    '',
