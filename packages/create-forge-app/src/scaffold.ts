...
async function rewriteProjectReadme(
  ...
  try {
    ...
    const generated = [
      ...
      '- Run `pnpm ops:index` when you want one inventory view of the current `ops/snapshots/`, `ops/evidence/`, `ops/reports/`, `ops/bundles/`, `ops/doctors/`, `ops/handoffs/`, `ops/attestations/`, `ops/ready/`, `ops/gates/`, `ops/releasepacks/`, `ops/exports/`, and `ops/restores/` directories',
      ...
      '- Run `pnpm ops:retention` to prune old `ops/snapshots/`, `ops/evidence/`, `ops/index/`, `ops/reports/`, `ops/bundles/`, `ops/doctors/`, `ops/handoffs/`, `ops/attestations/`, `ops/ready/`, `ops/gates/`, `ops/releasepacks/`, `ops/exports/`, and `ops/restores/` directories before or after repeated production checks',
...
function getProductionReadinessScript(): string {
  return [
    ...
    'done',
    '',
    'echo "=== Operations restore rehearsal (${snapshot_label}) ==="',
    'restore_args=(-- --label "$snapshot_label" --skip-retention)',
...
function getValidateWorkflow(): string {
  return [
    ...
    '        run: pnpm build',
    '',
    '      - name: Operations restore rehearsal',
    '        run: pnpm ops:restore -- --label validate --skip-retention',
    '',
...
function getReleaseWorkflow(): string {
  return [
    ...
    '',
    '      - name: Operations restore rehearsal',
    '        run: pnpm ops:restore -- --label release-${{ matrix.platform }} --skip-retention --require-release-output',
    '',
    '      - name: Upload operations snapshots',
    '        if: always()',
    '        uses: actions/upload-artifact@v4',
...
function getReleasePlaybook(projectName: string, metadata: ScaffoldMetadata): string {
  ...
  return [
    ...
    'pnpm production:check',
    'pnpm package',
    'pnpm ops:restore -- --label release-smoke --require-release-output',
    '```',
    '',
...
function getProductionReadinessGuide(projectName: string, metadata: ScaffoldMetadata): string {
  ...
  return [
    ...
    'pnpm production:check',
    'pnpm package',
    'pnpm ops:restore -- --label github-readiness --require-release-output',
    '```',
    '',
    ...
    'pnpm package:s3',
    'pnpm ops:restore -- --label s3-readiness --require-release-output',
    '```',
    '',
    ...
    'pnpm package',
    'pnpm ops:restore -- --label full-audit --require-release-output',
    '```',
    '',
    ...
    '',
    '## Final Restore Rehearsal',
    '',
    '```bash',
    'pnpm ops:restore -- --label release-restore --require-release-output',
    '```',
    '',
    'Use `pnpm ops:restore` when you want one final restore rehearsal under `ops/restores/` that rebuilds the latest offline export, rehydrates the payload, and leaves a Markdown and JSON proof after the gate verdict, release pack, and packaged release output are all in place.',
    '',
    '## What Gets Checked',
    '',
    ...
    '- Operations restore rehearsal under `ops/restores/` so operators get one final Markdown and JSON proof that the latest offline export can be unpacked and verified outside CI artifacts before handoff',
