...
async function rewriteProjectReadme(
  ...
  try {
    ...
    const generated = [
      ...
      '- Run `pnpm ops:index` when you want one inventory view of the current `ops/snapshots/`, `ops/evidence/`, `ops/reports/`, `ops/bundles/`, `ops/doctors/`, and `ops/handoffs/` directories',
      ...
      '- Run `pnpm ops:retention` to prune old `ops/snapshots/`, `ops/evidence/`, `ops/index/`, `ops/reports/`, `ops/bundles/`, `ops/doctors/`, and `ops/handoffs/` directories before or after repeated production checks',
...
function getProductionReadinessScript(): string {
  return [
    ...
    'echo "=== Operations snapshot (${snapshot_label}) ==="',
    'pnpm ops:snapshot -- --label "$snapshot_label"',
    'echo "=== Operations evidence bundle (${snapshot_label}) ==="',
    'pnpm ops:evidence -- --label "$snapshot_label" --skip-snapshot',
    'echo "=== Operations report (${snapshot_label}) ==="',
    'pnpm ops:report -- --label "$snapshot_label"',
    'echo "=== Operations bundle (${snapshot_label}) ==="',
    'pnpm ops:bundle -- --label "$snapshot_label"',
    'echo "=== Operations index (${snapshot_label}) ==="',
    'pnpm ops:index -- --label "$snapshot_label"',
    'echo "=== Operations doctor (${snapshot_label}) ==="',
    'pnpm ops:doctor -- --label "$snapshot_label"',
    'echo "=== Operations index refresh (${snapshot_label}) ==="',
    'pnpm ops:index -- --label "$snapshot_label"',
    'echo "=== Operations handoff (${snapshot_label}) ==="',
    'pnpm ops:handoff -- --label "$snapshot_label"',
    '',
...
function getOpsIndexScript(): string {
  return [
    ...
    '  counts: { snapshots: snapshots.length, evidence: evidence.length, reports: reports.length, bundles: bundles.length, doctors: doctors.length, handoffs: handoffs.length },',
    '  latest: {',
...
function getValidateWorkflow(): string {
  return [
    ...
    '        run: pnpm ops:snapshot -- --label validate',
    '',
    '      - name: Operations evidence bundle',
    '        run: pnpm ops:evidence -- --label validate --skip-snapshot',
    '',
    '      - name: Operations report',
    '        run: pnpm ops:report -- --label validate',
    '',
    '      - name: Operations bundle',
    '        run: pnpm ops:bundle -- --label validate',
    '',
    '      - name: Operations index',
    '        run: pnpm ops:index -- --label validate',
    '',
    '      - name: Operations doctor',
    '        run: pnpm ops:doctor -- --label validate',
    '',
    '      - name: Operations index refresh',
    '        run: pnpm ops:index -- --label validate',
    '',
    '      - name: Operations handoff',
    '        run: pnpm ops:handoff -- --label validate',
    '',
    '      - name: Type check',
    '        run: pnpm typecheck',
    '',
...
function getReleaseWorkflow(): string {
  return [
    ...
    '',
    '      - name: Operations snapshot',
    '        run: pnpm ops:snapshot -- --label release-${{ matrix.platform }}',
    '',
    '      - name: Operations evidence bundle',
    '        run: pnpm ops:evidence -- --label release-${{ matrix.platform }} --skip-snapshot',
    '',
    '      - name: Operations report',
    '        run: pnpm ops:report -- --label release-${{ matrix.platform }}',
    '',
    '      - name: Operations bundle',
    '        run: pnpm ops:bundle -- --label release-${{ matrix.platform }}',
    '',
    '      - name: Operations index',
    '        run: pnpm ops:index -- --label release-${{ matrix.platform }}',
    '',
    '      - name: Operations doctor',
    '        run: pnpm ops:doctor -- --label release-${{ matrix.platform }}',
    '',
    '      - name: Operations index refresh',
    '        run: pnpm ops:index -- --label release-${{ matrix.platform }}',
    '',
    '      - name: Operations handoff',
    '        run: pnpm ops:handoff -- --label release-${{ matrix.platform }}',
    '',
...
function getReleasePlaybook(projectName: string, metadata: ScaffoldMetadata): string {
  ...
  return [
    ...
    'pnpm ops:snapshot -- --label release-smoke',
    'pnpm ops:evidence -- --label release-smoke --skip-snapshot',
    'pnpm ops:report -- --label release-smoke',
    'pnpm ops:bundle -- --label release-smoke',
    'pnpm ops:index -- --label release-smoke',
    'pnpm ops:doctor -- --label release-smoke',
    'pnpm ops:index -- --label release-smoke',
    'pnpm ops:handoff -- --label release-smoke',
    'pnpm production:check',
    'pnpm package',
...
function getProductionReadinessGuide(projectName: string, metadata: ScaffoldMetadata): string {
  ...
  return [
    ...
    'pnpm ops:check',
    'pnpm ops:retention -- --keep 3',
    'pnpm ops:snapshot -- --label github-readiness',
    'pnpm ops:evidence -- --label github-readiness --skip-snapshot',
    'pnpm ops:report -- --label github-readiness',
    'pnpm ops:bundle -- --label github-readiness',
    'pnpm ops:index -- --label github-readiness',
    'pnpm ops:doctor -- --label github-readiness',
    'pnpm ops:index -- --label github-readiness',
    'pnpm ops:handoff -- --label github-readiness',
    'pnpm production:check',
    'pnpm package',
    'pnpm production:check:github -- --require-release-output',
    '```',
    ...
    'pnpm ops:check',
    'pnpm ops:retention -- --keep 3',
    'pnpm ops:snapshot -- --label s3-readiness',
    'pnpm ops:evidence -- --label s3-readiness --skip-snapshot',
    'pnpm ops:report -- --label s3-readiness',
    'pnpm ops:bundle -- --label s3-readiness',
    'pnpm ops:index -- --label s3-readiness',
    'pnpm ops:doctor -- --label s3-readiness',
    'pnpm ops:index -- --label s3-readiness',
    'pnpm ops:handoff -- --label s3-readiness',
    ...
    'pnpm ops:check',
    'pnpm ops:retention -- --keep 3',
    'pnpm ops:snapshot -- --label full-audit',
    'pnpm ops:evidence -- --label full-audit --skip-snapshot',
    'pnpm ops:report -- --label full-audit',
    'pnpm ops:bundle -- --label full-audit',
    'pnpm ops:index -- --label full-audit',
    'pnpm ops:doctor -- --label full-audit',
    'pnpm ops:index -- --label full-audit',
    'pnpm ops:handoff -- --label full-audit',
    ...
    '- Operations retention baseline so repeated production checks keep only the most recent snapshot, evidence, report, bundle, index, doctor, and handoff directories by default',
    ...
    '- Operations index under `ops/index/` so operators can inspect the current snapshot, evidence, report, bundle, and doctor inventory in one Markdown and JSON surface',
