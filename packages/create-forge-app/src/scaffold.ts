...
async function rewriteProjectReadme(
  ...
  try {
    ...
    const generated = [
      ...
      '- Run `pnpm ops:index` when you want one inventory view of the current `ops/snapshots/`, `ops/evidence/`, `ops/reports/`, `ops/bundles/`, `ops/doctors/`, `ops/handoffs/`, `ops/attestations/`, `ops/ready/`, `ops/gates/`, `ops/releasepacks/`, `ops/exports/`, `ops/restores/`, `ops/recoveries/`, `ops/rollbacks/`, `ops/incidents/`, and `ops/escalations/` directories',
...
function getProductionReadinessScript(): string {
  return [
    ...
    '',
    'echo "=== Operations escalation handoff (${snapshot_label}) ==="',
    'escalate_args=(-- --label "$snapshot_label" --skip-retention)',
    'if [ "$REQUIRE_RELEASE_OUTPUT" -eq 1 ]; then',
...
function getValidateWorkflow(): string {
  return [
    ...
    '        run: pnpm build',
    '',
    '      - name: Operations escalation handoff',
    '        run: pnpm ops:escalate -- --label validate --skip-retention',
    '',
...
function getReleaseWorkflow(): string {
  return [
    ...
    '',
    '      - name: Operations escalation handoff',
    '        run: pnpm ops:escalate -- --label release-${{ matrix.platform }} --skip-retention --require-release-output',
    '',
    '      - name: Upload operations snapshots',
    '        if: always()',
    '        uses: actions/upload-artifact@v4',
...
function getReleasePlaybook(projectName: string, metadata: ScaffoldMetadata): string {
  ...
  return [
    ...
    'pnpm production:check',
    'pnpm package',
    'pnpm ops:escalate -- --label release-smoke --require-release-output',
    '```',
    '',
...
function getProductionReadinessGuide(projectName: string, metadata: ScaffoldMetadata): string {
  ...
  return [
    ...
    'pnpm production:check',
    'pnpm package',
    'pnpm ops:escalate -- --label github-readiness --require-release-output',
    '```',
    '',
    ...
    'pnpm package:s3',
    'pnpm ops:escalate -- --label s3-readiness --require-release-output',
    '```',
    '',
    ...
    'pnpm package',
    'pnpm ops:escalate -- --label full-audit --require-release-output',
    '```',
    '',
    ...
    '- Operations retention baseline so repeated production checks keep only the most recent snapshot, evidence, report, bundle, index, doctor, handoff, attestation, ready, gate, release pack, export, restore, recover, rollback, incident, and escalation directories by default',
