...
async function rewriteProjectReadme(
  ...
  try {
    ...
    const generated = [
      ...
      '- Run `pnpm ops:index` when you want one inventory view of the current `ops/snapshots/`, `ops/evidence/`, `ops/reports/`, `ops/bundles/`, `ops/doctors/`, `ops/handoffs/`, `ops/attestations/`, `ops/ready/`, and `ops/gates/` directories',
      ...
      '- Run `pnpm ops:retention` to prune old `ops/snapshots/`, `ops/evidence/`, `ops/index/`, `ops/reports/`, `ops/bundles/`, `ops/doctors/`, `ops/handoffs/`, `ops/attestations/`, `ops/ready/`, and `ops/gates/` directories before or after repeated production checks',
...
function getProductionReadinessScript(): string {
  return [
    ...
    'echo "=== Operations gate (${snapshot_label}) ==="',
    'gate_args=(-- --label "$snapshot_label" --skip-retention)',
...
function getOpsIndexScript(): string {
  return [
    ...
    '  counts: { snapshots: snapshots.length, evidence: evidence.length, reports: reports.length, bundles: bundles.length, doctors: doctors.length, handoffs: handoffs.length, attestations: attestations.length, ready: readys.length, gates: gates.length },',
    '  latest: {',
...
function getValidateWorkflow(): string {
  return [
    ...
    '',
    '      - name: Operations gate pipeline',
    '        run: pnpm ops:gate -- --label validate --skip-retention',
    '',
    '      - name: Type check',
    '        run: pnpm typecheck',
    '',
...
function getReleaseWorkflow(): string {
  return [
    ...
    '',
    '      - name: Operations gate pipeline',
    '        run: pnpm ops:gate -- --label release-${{ matrix.platform }} --skip-retention',
    '',
...
function getReleasePlaybook(projectName: string, metadata: ScaffoldMetadata): string {
  ...
  return [
    ...
    'pnpm ops:gate -- --label release-smoke --skip-retention',
    'pnpm production:check',
    'pnpm package',
...
function getProductionReadinessGuide(projectName: string, metadata: ScaffoldMetadata): string {
  ...
  return [
    ...
    'pnpm ops:check',
    'pnpm ops:retention -- --keep 3',
    'pnpm ops:gate -- --label github-readiness --skip-retention',
    'pnpm production:check',
    'pnpm package',
    'pnpm production:check:github -- --require-release-output',
    '```',
    ...
    'pnpm ops:check',
    'pnpm ops:retention -- --keep 3',
    'pnpm ops:gate -- --label s3-readiness --skip-retention',
    ...
    'pnpm ops:check',
    'pnpm ops:retention -- --keep 3',
    'pnpm ops:gate -- --label full-audit --skip-retention',
    ...
    '',
    '```bash',
    'pnpm ops:gate -- --label release-gate --require-release-output',
    '```',
    '',
    'Use `pnpm ops:gate` when you want one final Markdown and JSON go/no-go verdict under `ops/gates/` after the ready chain and packaged release output are both in place.',
    '',
    '## What Gets Checked',
    '',
    ...
    '- Operations retention baseline so repeated production checks keep only the most recent snapshot, evidence, report, bundle, index, doctor, handoff, attestation, ready, and gate directories by default',
