...
async function rewriteProjectReadme(
  ...
  try {
    ...
    const generated = [
      ...
      '- Run `pnpm ops:index` when you want one inventory view of the current `ops/snapshots/` and `ops/evidence/` directories',
      ...
      '- Run `pnpm ops:retention` to prune old `ops/snapshots/`, `ops/evidence/`, `ops/index/`, and `ops/reports/` directories before or after repeated production checks',
...
function getOpsIndexScript(): string {
  return [
    ...
    '  counts: { snapshots: snapshots.length, evidence: evidence.length },',
    '  latest: {',
...
function getProductionReadinessGuide(projectName: string, metadata: ScaffoldMetadata): string {
  ...
  return [
    ...
    '- Operations retention baseline so repeated production checks keep only the most recent snapshot, evidence, index, and report directories by default',
    '- Operations index under `ops/index/` so operators can inspect the current snapshot and evidence inventory in one Markdown and JSON surface',
