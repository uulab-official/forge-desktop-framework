...
async function rewriteProjectReadme(
  ...
  try {
    ...
    const generated = [
      ...
      '- Run `pnpm ops:index` when you want one inventory view of the current `ops/snapshots/`, `ops/evidence/`, `ops/reports/`, `ops/bundles/`, `ops/doctors/`, `ops/handoffs/`, and `ops/ready/` directories',
      ...
      '- Run `pnpm ops:ready` when you want one production-grade command that refreshes snapshot, evidence, report, bundle, index, doctor, refreshed index, and handoff into a final ready verdict under `ops/ready/`',
      '- Run `pnpm ops:retention` to prune old `ops/snapshots/`, `ops/evidence/`, `ops/index/`, `ops/reports/`, `ops/bundles/`, `ops/doctors/`, `ops/handoffs/`, and `ops/ready/` directories before or after repeated production checks',
...
function getProductionReadinessScript(): string {
  return [
    ...
    'echo "=== Operations ready (${snapshot_label}) ==="',
    'ready_args=(-- --label "$snapshot_label" --skip-retention)',
...
function getOpsIndexScript(): string {
  return [
    ...
    '  counts: { snapshots: snapshots.length, evidence: evidence.length, reports: reports.length, bundles: bundles.length, doctors: doctors.length, handoffs: handoffs.length, ready: readys.length },',
    '  latest: {',
...
function getValidateWorkflow(): string {
  return [
    ...
    '',
    '      - name: Operations ready pipeline',
    '        run: pnpm ops:ready -- --label validate --skip-retention',
    '',
    '      - name: Type check',
    '        run: pnpm typecheck',
    '',
...
function getReleaseWorkflow(): string {
  return [
    ...
    '',
    '      - name: Operations ready pipeline',
    '        run: pnpm ops:ready -- --label release-${{ matrix.platform }} --skip-retention',
    '',
...
function getReleasePlaybook(projectName: string, metadata: ScaffoldMetadata): string {
  ...
  return [
    ...
    'pnpm ops:ready -- --label release-smoke --skip-retention',
    'pnpm production:check',
    'pnpm package',
...
function getProductionReadinessGuide(projectName: string, metadata: ScaffoldMetadata): string {
  ...
  return [
    ...
    'pnpm ops:check',
    'pnpm ops:retention -- --keep 3',
    'pnpm ops:ready -- --label github-readiness --skip-retention',
    'pnpm production:check',
    'pnpm package',
    'pnpm production:check:github -- --require-release-output',
    '```',
    ...
    'pnpm ops:check',
    'pnpm ops:retention -- --keep 3',
    'pnpm ops:ready -- --label s3-readiness --skip-retention',
    ...
    'pnpm ops:check',
    'pnpm ops:retention -- --keep 3',
    'pnpm ops:ready -- --label full-audit --skip-retention',
    ...
    '- Operations retention baseline so repeated production checks keep only the most recent snapshot, evidence, report, bundle, index, doctor, handoff, and ready directories by default',
    ...
    '- Operations index under `ops/index/` so operators can inspect the current snapshot, evidence, report, bundle, doctor, handoff, and ready inventory in one Markdown and JSON surface',
    ...
    '- Operations ready summary under `ops/ready/` so operators get one final Markdown and JSON production verdict that refreshes the full ops chain in one command',
